...
async function compareProduccionVsLocal() {
  ...
  try {
    ...
    try {
      const produccionResponse = await fetch(`${produccionUrl}&_t=${Date.now()}`, {
        headers: {
      ...
      if (produccionData.success && produccionData.data) {
        ...
        const prodConValor = produccionData.data.filter(p => p.valor_proyectado > 0);
        const prodCero = produccionData.data.filter(p => p.valor_proyectado === 0);
        ...
        if (prodConValor.length > 0) {
          ...
          prodConValor.sort((a, b) => b.valor_proyectado - a.valor_proyectado)
            .slice(0, 3).forEach((p, i) => {
              console.log(`   ${i + 1}. Item ${p.item}: $${p.valor_proyectado.toLocaleString('es-CO')}`);
      ...
      log('   ⚠️ Verifica la URL de producción o que esté desplegado', 'yellow');
    }
    ...
    try {
      ...
      if (localData.success && localData.data) {
        ...
        const localConValor = localData.data.filter(p => p.valor_proyectado > 0);
        const localCero = localData.data.filter(p => p.valor_proyectado === 0);
        ...
        if (localConValor.length > 0) {
          ...
          localConValor.sort((a, b) => b.valor_proyectado - a.valor_proyectado)
            .slice(0, 3).forEach((p, i) => {
              console.log(`   ${i + 1}. Item ${p.item}: $${p.valor_proyectado.toLocaleString('es-CO')}`);
              console.log(`      Nombre: ${p.nombre_resumido_proceso || 'N/A'}`);
            });
      ...
      log('   ⚠️ Asegúrate de que el servidor local esté corriendo (npm run dev)', 'yellow');
    }
    ...
    if (produccionData && localData) {
      ...
      console.log(`   Diferencia: ${Math.abs(prodConValor.length - localConValor.length)} registros`);
      
      if (prodConValor.length === 0 && localConValor.length > 0) {
        log(`\n❌ PROBLEMA CONFIRMADO: Producción tiene ceros, Local tiene valores`, 'red');
        ...
        if (prodItem1 && localItem1) {
          ...
          if (produccionData.timestamp !== localData.timestamp) {
            log(`   ⚠️ Timestamps diferentes - están usando datos de diferentes momentos`, 'yellow');
          }
          
          // Verificar variables de entorno
          log(`\n🔍 Posibles causas:`, 'yellow');
          console.log(`   1. Variables de entorno diferentes (NEXT_PUBLIC_API_BASE_URL)`);
      ...
      } else if (prodConValor.length === localConValor.length && prodConValor.length > 0) {
    ...
    if (railwayData.success && railwayData.data) {
      const railwayConValor = railwayData.data.filter(p => p.valor_proyectado > 0);
      ...
      if (railwayConValor.length > 0) {
        ...
        railwayConValor.sort((a, b) => b.valor_proyectado - a.valor_proyectado)
          .slice(0, 3).forEach((p, i) => {
            console.log(`   ${i + 1}. Item ${p.item}: $${p.valor_proyectado.toLocaleString('es-CO')}`);
          });
      }
    ...
    if (localData && localConValor && railwayConValor) {
      const localTieneValores = localData.data.filter(p => p.valor_proyectado > 0).length > 0;
      const railwayTieneValores = railwayConValor.length > 0;
      const produccionTieneValores = produccionData ? produccionData.data.filter(p => p.valor_proyectado > 0).length > 0 : false;
      
      if (railwayTieneValores && localTieneValores && !produccionTieneValores) {
        log(`\n❌ PROBLEMA: Producción no recibe datos correctos del backend`, 'red');
        log(`\n📋 Acciones:`, 'yellow');
        console.log(`   1. Verificar NEXT_PUBLIC_API_BASE_URL en Vercel`);
        console.log(`   2. Limpiar cache de Vercel (Dashboard > Deployments > ... > Redeploy)`);
        ...
        log(`\n⚠️ Backend Railway devuelve ceros - problema en Google Sheet`, 'yellow');
      } else {
