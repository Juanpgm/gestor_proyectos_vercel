...
async function testCacheProyecciones() {
  ...
  try {
    ...
    if (uniqueTimestamps.size === 3) {
      log('\n✅ BACKEND SIN CACHE: Cada llamada tiene timestamp diferente', 'green');
    } else {
    ...
    if (uniqueCounts.size === 1) {
      log(`✅ DATOS CONSISTENTES: ${dataCounts[0]} registros en todas las llamadas`, 'green');
    } else {
      log(`⚠️ DATOS INCONSISTENTES: Diferentes cantidades de registros`, 'yellow');
      dataCounts.forEach((count, i) => console.log(`   Llamada ${i + 1}: ${count} registros`));
    }
    ...
    try {
      ...
      if (uniqueLocalTimestamps.size === 3) {
        log('\n✅ PROXY SIN CACHE: Cada llamada obtiene datos frescos del backend', 'green');
      } else if (uniqueLocalTimestamps.size === 1) {
        log('\n❌ PROXY CON CACHE: Todas las llamadas tienen el mismo timestamp', 'red');
        log('   Esto significa que el proxy está cacheando las respuestas', 'red');
      } else {
        log('\n⚠️ PROXY CON CACHE PARCIAL: Algunos timestamps se repiten', 'yellow');
      }
    ...
    try {
      ...
      log(`✅ Proyecciones sin proceso: ${data.data?.length || 0} registros`, 'green');
      console.log(`   Timestamp: ${data.timestamp}`);
      
      // Verificar con el proxy local
      try {
        const localSinProcesoUrl = 'http://localhost:3000/api/emprestito/proyecciones-sin-proceso';
        const localResponse = await fetch(`${localSinProcesoUrl}?_t=${Date.now()}`);
        const localData = await localResponse.json();
        
        log(`✅ Proxy local - sin proceso: ${localData.data?.length || 0} registros`, 'green');
        
        if (data.data?.length === localData.data?.length) {
          log('✅ Mismo número de registros sin proceso en ambos endpoints', 'green');
        } else {
    ...
    log('   1. Verificar headers de Cache-Control en las rutas de API', 'yellow');
    ...
    log('   1. Verificar que NEXT_PUBLIC_API_BASE_URL esté configurado', 'magenta');
    log('   2. Limpiar cache de Vercel después del deploy', 'magenta');
    log('   3. Usar el parámetro ?_t=timestamp en todas las llamadas', 'magenta');
