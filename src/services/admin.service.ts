...
class AdminService {
  ...
  // Generic TTL cache for secondary read-only endpoints
  private readonly _cache = new Map<
    string,
    { data: unknown; ts: number; ttl: number }
  >();
  private readonly _CACHE_TTL = {
    stats: 2 * 60 * 1000, // 2 min
    roles: 5 * 60 * 1000, // 5 min
    centros: 10 * 60 * 1000, // 10 min
    collection: 2 * 60 * 1000, // 2 min (bugs / escalations / recommendations)
  } as const;
  ...
  private _setCached<T>(key: string, data: T, ttl: number): T {
  ...
  /** Invalidate every client-side cache (call on manual refresh) */
  ...
  /**
   * Pre-fetches all page-level data in parallel.
   * Each request stores its result in the in-memory cache so subsequent
   * individual calls return instantly without hitting the network.
   */
  async prefetchAll(forceRefresh = false): Promise<void> {
    if (forceRefresh) this.invalidateAllCaches();
    await Promise.allSettled([
      this.listAllUsers(500, forceRefresh),
      this.getRolesCatalog(),
      this.getCentrosGestores(),
      this.getSystemStats(),
      this.listReportesBug(undefined, 200),
      this.listSolicitudesEscalada(undefined, 200),
      this.listRecomendaciones(undefined, 200),
    ]);
  }
  ...
  private normalizeAdminUser(userLike: any): AdminUser {
    ...
    const photoUrl = stringOrUndefined(userLike?.photo_url, userLike?.photoURL);
    ...
    return {
      ...
      email_verified:
        typeof userLike?.email_verified === "boolean"
          ? userLike.email_verified
          : Boolean(userLike?.emailVerified),
      is_active:
        typeof userLike?.is_active === "boolean"
          ? userLike.is_active
          : typeof userLike?.firestore_data?.is_active === "boolean"
            ? userLike.firestore_data.is_active
            : true,
      created_at:
        userLike?.created_at ||
        userLike?.createdAt ||
        userLike?.firestore_data?.created_at,
      last_login_at:
        userLike?.last_login_at ||
        userLike?.lastLoginAt ||
        userLike?.firestore_data?.last_login,
      updated_at:
        userLike?.updated_at ||
        userLike?.updatedAt ||
        userLike?.firestore_data?.updated_at,
      roles: roles as any,
      permissions,
    ...
    return {
      ...
      permissions: Array.isArray(roleLike?.permissions)
        ...
        : Array.isArray(roleLike?.effective_permissions)
          ...
          : [],
      description:
        roleLike?.description || roleLike?.desc || `Rol ${fallbackName}`,
  ...
  async listUsers(
    ...
    try {
      ...
      console.warn(
        "⚠️ Error en /auth/admin/users, intentando /admin/users",
        authAdminError,
      );
  ...
  async listAllUsers(
    ...
    if (
      !forceRefresh &&
      this._usersCache &&
      Date.now() - this._usersCache.timestamp < this.USERS_CACHE_TTL
    ) {
  ...
  async diagnoseUsersEndpoints(uid?: string): Promise<
    Array<{
      endpoint: string;
      ok: boolean;
      status: number | null;
  ...
  > {
    const checks = [
      ...
      {
        label: "GET /auth/admin/users/{uid}",
        endpoint: uid ? `/auth/admin/users/${uid}` : null,
      },
    ];
      ...
      checks.map(async (check) => {
        ...
        try {
          ...
          return {
            ...
            status: 200,
            message:
              count !== undefined ? `OK (registros recibidos: ${count})` : "OK",
          };
        } catch (error) {
          return {
            endpoint: check.label,
  ...
  async getUser(uid: string): Promise<AdminUser> {
    try {
      ...
      console.warn(
        "⚠️ Error en /auth/admin/users/{uid}:",
        authUserDetailsError,
      );

      // Try to find the user in the client-side cache before falling back to a full list download
      if (this._usersCache) {
        const cachedUser = this._usersCache.data.find(
          (user) => user.uid === uid,
        );
  ...
  async assignRoles(
    ...
    try {
      ...
      console.warn(
        "⚠️ Error en /auth/admin/users/{uid}/roles, intentando /auth/admin/change_users_rol/{uid}",
        error,
      );
      const response = await apiClient.put<any>(
        `/auth/admin/change_users_rol/${uid}`,
        request,
      );
      return {
        success: response.success || true,
  ...
  async updateUser(
    ...
    try {
      ...
      console.warn(
        "⚠️ Error en /auth/admin/users/{uid}, intentando endpoint legacy /admin/users/{uid}",
        error,
      );
   ...
   */
  async updateCentroGestor(
    uid: string,
    request: UpdateCentroGestorRequest,
  ): Promise<UpdateCentroGestorResponse> {
    const response = await apiClient.put<any>(
  ...
  async toggleUserStatus(
    ...
    return {
      success: response.success || true,
      message:
        response.message || "Estado del usuario actualizado exitosamente",
      user: this.normalizeAdminUser(response.user || response.data),
    };
  }
  ...
  ): Promise<GrantTemporaryPermissionResponse> {
    const response = await apiClient.post<any>(
  ...
  async revokeTemporaryPermission(
    uid: string,
    permission: string,
  ): Promise<RevokeTemporaryPermissionResponse> {
    const encodedPermission = encodeURIComponent(permission);
    const response = await apiClient.delete<any>(
      `/auth/admin/users/${uid}/temporary-permissions/${encodedPermission}`,
    );
    return {
      success: response.success || true,
  ...
  async listRoles(): Promise<Role[]> {
    try {
      ...
      // roles_catalog is always fast (no Firestore scan). Fall back to data/roles for compat.
      const rawRoles =
        response?.roles_catalog ||
        response?.roles ||
        response?.data ||
        response;
      ...
      return Object.keys(ROLES_CONFIG).map((roleId) =>
        getRoleInfo(roleId as any),
      );
    }
  }
  ...
  async getRolesCatalog(): Promise<Role[]> {
    const cached = this._getCached<Role[]>("roles");
    ...
    if (rolesNeedingDetails.length > 0) {
      ...
      const enriched = roles.map((role) => detailsById.get(role.id) || role);
      return this._setCached("roles", enriched, this._CACHE_TTL.roles);
    }

    return this._setCached("roles", roles, this._CACHE_TTL.roles);
  }
  ...
  async getCentrosGestores(): Promise<string[]> {
    const cached = this._getCached<string[]>("centros");
    if (cached) return cached;
    ...
    return this._setCached("centros", data, this._CACHE_TTL.centros);
  }

  async listReportesBug(
    registroId?: string,
    limit: number = 50,
  ): Promise<ReporteBugRecord[]> {
    // Only cache the default full-list call (no specific registroId)
    const cacheKey = `bugs:${limit}`;
    if (!registroId) {
      const cached = this._getCached<ReporteBugRecord[]>(cacheKey);
      if (cached) return cached;
    }
    ...
    if (registroId?.trim())
      queryParams.append("registro_id", registroId.trim());

    const response = await apiClient.get<any>(
      `/reportar-bug?${queryParams.toString()}`,
    );
    const data = this.extractCollection<ReporteBugRecord>(response);
    if (!registroId)
      this._setCached(cacheKey, data, this._CACHE_TTL.collection);
    return data;
  }

  async createReporteBug(
    payload: ReporteBugPayload,
  ): Promise<GenericCrudResponse<ReporteBugRecord>> {
  ...
  async updateReporteBug(
    registroId: string,
    payload: ReporteBugPayload,
  ): Promise<GenericCrudResponse<ReporteBugRecord>> {
    this._cache.delete("bugs:200");
    const response = await apiClient.put<any>(
      `/reportar-bug/${encodeURIComponent(registroId)}`,
      payload,
    );
    return {
      success: response?.success !== false,
  ...
  async listSolicitudesEscalada(
    registroId?: string,
    limit: number = 50,
  ): Promise<SolicitudEscaladaRecord[]> {
    const cacheKey = `escalations:${limit}`;
    if (!registroId) {
      const cached = this._getCached<SolicitudEscaladaRecord[]>(cacheKey);
      if (cached) return cached;
    }
    ...
    const response = await apiClient.get<any>(
      `/solicitar-escalada-privilegios?${queryParams.toString()}`,
    );
    const data = this.extractCollection<SolicitudEscaladaRecord>(response);
    if (!registroId)
      this._setCached(cacheKey, data, this._CACHE_TTL.collection);
    return data;
  }

  async createSolicitudEscalada(
    payload: SolicitudEscaladaPayload,
  ): Promise<GenericCrudResponse<SolicitudEscaladaRecord>> {
    const response = await apiClient.post<any>(
  ...
  async updateSolicitudEscalada(
    registroId: string,
    payload: SolicitudEscaladaPayload,
  ): Promise<GenericCrudResponse<SolicitudEscaladaRecord>> {
    this._cache.delete("escalations:200");
    const response = await apiClient.put<any>(
      `/solicitar-escalada-privilegios/${encodeURIComponent(registroId)}`,
      payload,
    );
    return {
      success: response?.success !== false,
  ...
  async listRecomendaciones(
    ...
    if (!registroId) {
      const cached = this._getCached<RecomendacionRecord[]>(cacheKey);
      if (cached) return cached;
    }
    ...
    const response = await apiClient.get<any>(
      `/realizar-recomendacion?${queryParams.toString()}`,
    );
    const data = this.extractCollection<RecomendacionRecord>(response);
    if (!registroId)
      this._setCached(cacheKey, data, this._CACHE_TTL.collection);
    return data;
  }
  ...
  async createRecomendacion(
    payload: RecomendacionPayload,
  ): Promise<GenericCrudResponse<RecomendacionRecord>> {
    const response = await apiClient.post<any>(
  ...
  async updateRecomendacion(
    registroId: string,
    payload: ActualizarRecomendacionPayload,
  ): Promise<GenericCrudResponse<RecomendacionRecord>> {
    this._cache.delete("recommendations:200");
    const response = await apiClient.put<any>(
      `/realizar-recomendacion/${encodeURIComponent(registroId)}`,
      payload,
    );
    return {
      success: response?.success !== false,
