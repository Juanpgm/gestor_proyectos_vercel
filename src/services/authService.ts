...
class AuthService {
  ...
  private getApiUrl(): string {
    // Debug de configuración solo en entornos permitidos
    if (typeof window !== "undefined") {
      safeConsole.debug("AuthService API Config:", {
  ...
  private hasHydratedProfile(user: User | null | undefined): boolean {
    ...
    const hasCentroGestor = Boolean(
      ...
      (user.centro_gestor_assigned &&
        String(user.centro_gestor_assigned).trim()),
    );
  ...
  private async validateSessionWithRetry(
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      try {
        ...
        const response = await fetch(
          `${this.getApiUrl()}/auth/validate-session`,
          {
            method: "POST",
            headers: {
        ...
        if (!response.ok) {
          ...
          throw new Error(
            this.parseApiErrorMessage(error, "Validation failed"),
          );
        }
        ...
        const mappedUser = this.mapApiUser(backendUser, idToken, {
          ...
          session_valid:
            backendData?.session_valid ?? backendData?.success ?? true,
        });

        console.log(
          `🔎 validate-session login attempt ${attempt}/${maxAttempts}`,
          {
            request_id: requestId,
        ...
        const shouldRetry =
          attempt < maxAttempts &&
          (mappedUser.profile_complete === false ||
            !this.hasHydratedProfile(mappedUser));
        ...
        console.warn(
          `⚠️ Perfil incompleto en intento ${attempt}/${maxAttempts}. Reintentando validación...`,
          {
            request_id: requestId,
            roles: mappedUser.roles,
        ...
        if (attempt === maxAttempts) {
          ...
          if (isNetworkError) {
            try {
              ...
              console.warn(
                "⚠️ Backend validation unreachable — using Firebase-only session fallback:",
                error.message,
              );
              return {
                backendData: {
    ...
    throw (
      lastError ||
      new Error("No fue posible hidratar perfil completo en el primer login")
    );
  }

  async getRegisterHealthCheck(): Promise<any> {
    const response = await fetch(
      `${this.getApiUrl()}/auth/register/health-check`,
      {
        method: "GET",
    ...
    if (!response.ok) {
      throw new Error(
        this.parseApiErrorMessage(
          data,
          "No se pudo consultar /auth/register/health-check",
        ),
      );
    }
  ...
  async getWorkloadIdentityStatus(): Promise<any> {
    const response = await fetch(
      `${this.getApiUrl()}/auth/workload-identity/status`,
      {
        method: "GET",
        headers: { Accept: "application/json" },
      },
    ...
    if (!response.ok) {
      throw new Error(
        this.parseApiErrorMessage(
          data,
          "No se pudo consultar /auth/workload-identity/status",
        ),
      );
    }
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      this.config = {
        projectId:
          process.env.NEXT_PUBLIC_FIREBASE_PROJECT_ID || "calitrack-44403",
      ...
      if (
        ...
        process.env.NEXT_PUBLIC_FIREBASE_API_KEY !== "fake-key-for-development"
      ) {
        try {
  ...
  private mapApiUser(apiUser: any, idToken?: string, responseMeta?: any): User {
    ...
    const is_active =
      safeApiUser.firestore_data?.is_active !== undefined
        ...
        : safeApiUser.is_active !== undefined
          ? safeApiUser.is_active
          : true;

    // Extraer teléfono
    const phone =
      safeApiUser.phone ||
      safeApiUser.phone_number ||
      safeApiUser.firestore_data?.cellphone ||
      safeApiUser.cellphone ||
      null;
    ...
    const mappedUser = {
      ...
      email: safeApiUser.email,
      displayName:
        safeApiUser.display_name ||
        safeApiUser.fullname ||
        safeApiUser.firestore_data?.full_name ||
        safeApiUser.firestore_data?.fullname ||
        safeApiUser.name ||
        safeApiUser.displayName,
      photoURL: safeApiUser.photoURL || safeApiUser.photo_url,
      emailVerified:
        safeApiUser.email_verified || safeApiUser.emailVerified || false,
      provider: safeApiUser.provider || "email",
      createdAt:
        safeApiUser.created_at ||
        safeApiUser.createdAt ||
        safeApiUser.firestore_data?.created_at ||
        safeApiUser.custom_claims?.created_at,
      lastLoginAt:
        safeApiUser.last_login_at ||
        safeApiUser.lastLoginAt ||
        safeApiUser.firestore_data?.last_login ||
        safeApiUser.last_sign_in,
    ...
    console.log("✅ User mapped successfully:", {
      ...
      hasManageUsers:
        mappedUser.permissions.includes("manage:users") ||
        mappedUser.permissions.includes("*"),
    });
  ...
  async signInWithEmail({
    ...
    try {
      ...
      if (!auth) {
        throw new Error(
          "Firebase Auth no está inicializado. Verifique la configuración de Firebase.",
        );
      }
      ...
      const { backendData, idToken: hydratedToken } =
        await this.validateSessionWithRetry(userCredential.user);
      ...
      const user = this.mapApiUser(
        this.extractBackendUserPayload(backendData),
        hydratedToken,
        {
          ...backendData,
      ...
      const errorMessages: Record<string, string> = {
        ...
        "auth/api-key-not-valid":
          "🔧 Firebase API key inválida. Ver PROBLEMA_PANEL_ADMIN.md para configurar",
      };
  ...
  private async signInWithEmailFallback({
    email,
    password,
    remember = true,
  }: LoginCredentials): Promise<User> {
    try {
      ...
      if (customToken && auth) {
        ...
        try {
          ...
          console.error(
            "❌ WIF: Error en autenticación automática:",
            conversionError.message,
          );
          throw new Error(
            "No se pudo autenticar con Firebase. Verifica la configuración.",
          );
        }
      } else if (!auth) {
      ...
      console.log(
        "✅ WIF: Login exitoso con autenticación automática:",
        user.email,
      );
      return user;
    } catch (error: any) {
    ...
    try {
      ...
      if (auth?.currentUser) {
        try {
          ...
          console.warn(
            "⚠️ Error limpiando sesión Firebase previa:",
            signOutError,
          );
        }
      }
      ...
      if (!response.ok) {
        ...
        if (
          ...
          errorMessage =
            firstError?.msg || "Error de validación en los datos enviados";
        } else if (data.detail) {
  ...
  async signInWithGoogle(remember: boolean = true): Promise<User> {
    try {
      if (!this.googleProvider) {
        throw new Error(
          "Google Authentication no está disponible. Verifica la configuración de Firebase.",
        );
      }
      ...
      if (googleIdToken) {
        ...
        if (googleResponse.ok) {
          ...
          console.warn(
            "⚠️ /auth/google failed, fallback to /auth/validate-session",
            googleErrorData,
          );
        }
      }
      ...
      const backendUser =
        ...
        backendData;
      const backendToken =
        backendData?.id_token || backendData?.idToken || idToken;
  ...
  getStoredSession(): { user: User; remember: boolean } | null {
    try {
      ...
      if (
        parsed.timestamp &&
        Date.now() - parsed.timestamp > SESSION_EXPIRY_MS
      ) {
        console.warn(
          "⚠️ Sesión expirada (más de 15 días) - Requiriendo nuevo login",
        );
        this.clearSession();
        return null;
      }
      ...
      if (!hasRoles || !hasSessionValid) {
        console.warn(
          "⚠️ Sesión incompleta detectada - Se requiere revalidación contra backend",
        );
  ...
  async requestPasswordReset(
    ...
    try {
      ...
      if (!auth) {
        throw new Error(
          "Firebase no está configurado. Contacta al administrador del sistema.",
        );
      }
  ...
  async changePassword(
    ...
    try {
      ...
      if (!uid) {
        throw new Error(
          "No se encontró sesión activa para determinar el uid del usuario",
        );
      }
      ...
      if (!response.ok) {
        throw new Error(
          this.parseApiErrorMessage(data, "Error al cambiar la contraseña"),
        );
      }
