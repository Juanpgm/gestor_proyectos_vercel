...
const AVANCES_UNIDADES_PROYECTO_ENDPOINT =
  "/api/proxy/avances_unidades_proyecto";
...
const generateId = () =>
  `avance-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
...
const mapApiAvanceToAvanceUP = (
  apiAvance: Record<string, any>,
  upid: string,
): AvanceUP => {
  const createdAt =
    typeof apiAvance?.created_at === "string"
      ? apiAvance.created_at
      : new Date().toISOString();
  const avanceObra = Number(apiAvance?.avance_obra ?? 0);
  const observaciones =
    typeof apiAvance?.observaciones === "string" ? apiAvance.observaciones : "";

  // New schema: soportes[] with tipo: "imagen" | "documento"
  const soportes: SoporteUP[] = Array.isArray(apiAvance?.soportes)
    ? apiAvance.soportes
        .filter(
          (s: unknown): s is Record<string, any> =>
            typeof s === "object" && s !== null,
        )
        .map((s: Record<string, any>) => {
          // Prefer presigned URL (direct S3 access) over raw URL
          const effectiveUrl: string =
            (typeof s.url_presigned === "string" && s.url_presigned.trim()
              ? s.url_presigned.trim()
              : "") ||
            (typeof s.presigned_url === "string" && s.presigned_url.trim()
              ? s.presigned_url.trim()
              : "") ||
            String(s.url ?? "");
          // Extract s3_key from s3_key field or derive from url_directa
          const urlDirecta: string | undefined =
            typeof s.url_directa === "string" && s.url_directa.trim()
              ? s.url_directa.trim()
              : undefined;
          const derivedKey =
            typeof s.s3_key === "string" && s.s3_key.trim()
              ? s.s3_key.trim()
              : urlDirecta
          ...
          return {
            ...
            url_presigned:
              typeof s.url_presigned === "string" ? s.url_presigned : undefined,
            presigned_url:
              typeof s.presigned_url === "string" ? s.presigned_url : undefined,
  ...
  const imagenes_urls: string[] = (() => {
    ...
    if (Array.isArray(apiAvance?.imagenes_urls))
      return apiAvance.imagenes_urls.filter(
        (u: unknown): u is string => typeof u === "string" && u.length > 0,
      );
    if (Array.isArray(apiAvance?.registro_fotografico_urls))
      return apiAvance.registro_fotografico_urls.filter(
        (u: unknown): u is string => typeof u === "string" && u.length > 0,
      );
    return [];
  })();
  ...
  const documentos_urls: string[] = (() => {
    ...
    if (Array.isArray(apiAvance?.documentos_urls))
      return apiAvance.documentos_urls.filter(
        (u: unknown): u is string => typeof u === "string" && u.length > 0,
      );
    if (Array.isArray(apiAvance?.documentos_soporte_urls))
      return apiAvance.documentos_soporte_urls.filter(
        (u: unknown): u is string => typeof u === "string" && u.length > 0,
      );
    return [];
  })();
    ...
    soportes.length > 0
      ...
      : [...imagenes_urls, ...documentos_urls].map((url, index) => {
          ...
          return {
            ...
            nombre: filename,
            tipo: imagenes_urls.includes(url)
              ? "image/*"
  ...
  return {
    ...
    reportado_por:
      typeof apiAvance?.registrado_por === "string"
        ? apiAvance.registrado_por
        : "Sistema",
    archivos,
    soportes,
    imagenes_urls,
    ...
    updated_at:
      typeof apiAvance?.updated_at === "string"
        ? apiAvance.updated_at
        : createdAt,
  };
};
...
export const useAvancesUP = (upid: string, intervencionId?: string) => {
  ...
  const getIntervencionIdByUpid = useCallback(async (): Promise<
    string | null
  > => {
    ...
    try {
      const response = await proxyFetch(
        `/api/proxy/intervenciones?upid=${encodeURIComponent(upid)}&limit=1`,
        {
          method: "GET",
          cache: "no-store",
        },
  ...
  const loadAvances = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `Error ${response.status} al consultar historial de avances`,
        );
      }
  ...
  const addAvance = useCallback(
    async (formData: AvanceUPFormData): Promise<boolean> => {
      try {
        // Validaciones
        if (formData.avance_fisico < 0 || formData.avance_fisico > 100) {
          setState((prev) => ({
            ...prev,
            error: "El avance físico debe estar entre 0 y 100",
          }));
          return false;
        }
        if (
          formData.avance_financiero < 0 ||
          formData.avance_financiero > 100
        ) {
          setState((prev) => ({
            ...prev,
            error: "El avance financiero debe estar entre 0 y 100",
          }));
          return false;
        }
        ...
        if (!apiResponse.ok) {
          ...
          throw new Error(
            errorData?.detail ||
              errorData?.error ||
              `Error ${apiResponse.status} al registrar avance`,
          );
        }
  ...
  const editAvance = useCallback(
    (avanceId: string, formData: Partial<AvanceUPFormData>): boolean => {
      try {
        const allData = loadFromStorage();
        ...
        avancesUP[index] = {
          ...avancesUP[index],
          ...(formData.fecha_reporte && {
            fecha_reporte: formData.fecha_reporte,
          }),
          ...(formData.avance_fisico !== undefined && {
            avance_fisico: formData.avance_fisico,
          }),
          ...(formData.avance_financiero !== undefined && {
            avance_financiero: formData.avance_financiero,
          }),
          ...(formData.valor_ejecutado !== undefined && {
  ...
  const deleteAvance = useCallback(
    async (avanceId: string): Promise<boolean> => {
      const normalizedId = String(avanceId || "").trim();
      if (!normalizedId) {
        setState((prev) => ({
          ...prev,
          error: "ID de avance inválido para eliminar",
        }));
        return false;
      }
      ...
      const deleteFromLocalStorage = (): boolean => {
        try {
          ...
          setState((prev) => ({
            ...prev,
            error: "Error al eliminar el avance localmente",
          }));
          return false;
        }
      };
      ...
      try {
        ...
        if (resolvedIntervencionId) {
          queryCandidates.add(
            `id=${encodeURIComponent(normalizedId)}&intervencion_id=${encodeURIComponent(resolvedIntervencionId)}`,
          );
          queryCandidates.add(
            `avance_id=${encodeURIComponent(normalizedId)}&intervencion_id=${encodeURIComponent(resolvedIntervencionId)}`,
          );
        }
        ...
        for (const query of Array.from(queryCandidates)) {
          const response = await proxyFetch(
            `${AVANCES_UNIDADES_PROYECTO_ENDPOINT}?${query}`,
            {
              method: "DELETE",
              cache: "no-store",
            },
          ...
          lastError =
            rawError?.trim() || `Error ${response.status} al eliminar avance`;
        ...
        const bodyResponse = await proxyFetch(
          AVANCES_UNIDADES_PROYECTO_ENDPOINT,
          {
            method: "DELETE",
            headers: {
        ...
        lastError =
          rawBodyError?.trim() ||
          `Error ${bodyResponse.status} al eliminar avance`;
