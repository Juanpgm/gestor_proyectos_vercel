...
export function useReportesCentroGestorDashboard() {
  const {
    ...
    error,
    refetch,
  } = useReportesCentroGestor();
  const { centrosGestor: todosLosCentros, loading: loadingCentros } =
    useCentrosGestorFromContratos();
  ...
  const resumenPorCentroGestor = useMemo((): CentroGestorResumen[] => {
    ...
    for (const r of reportes) {
      ...
      if (fecha) {
        if (
          !resumen.ultimo_reporte ||
          fecha > new Date(resumen.ultimo_reporte)
        ) {
        ...
        if (
          !resumen.primer_reporte ||
          fecha < new Date(resumen.primer_reporte)
        ) {
  ...
  const centrosSinReportes = useMemo(() => {
    ...
    const conReportesSet = new Set(
      centrosConReportes.map((c) => c.toLowerCase().trim()),
    );
    return todosLosCentros.filter(
      (c) => !conReportesSet.has(c.toLowerCase().trim()),
    );
