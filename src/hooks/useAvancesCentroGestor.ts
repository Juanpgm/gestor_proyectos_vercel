...
export function useAvancesCentroGestor() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (intervencionesRes.status === 'fulfilled' && intervencionesRes.value.ok) {
  ...
  const intervencionesConAvances = useMemo((): IntervencionConAvances[] => {
    return intervenciones.map(int => {
      ...
      const completada = avanceObra >= 100 || estado === 'Terminado' || estado === 'Liquidado'
      return {
        intervencion_id: int.intervencion_id,
        upid: int.upid,
        nombre_centro_gestor: (int.nombre_centro_gestor || 'Sin centro gestor').trim(),
        estado,
  ...
  const resumenPorCentroGestor = useMemo((): CentroGestorAvancesResumen[] => {
    ...
    for (const int of intervencionesConAvances) {
      ...
      if (int.tiene_avances) {
        ...
        for (const av of int.avances) {
          ...
          if (fecha) {
            if (!resumen.ultimo_avance || fecha > new Date(resumen.ultimo_avance)) {
              resumen.ultimo_avance = av.created_at
            }
            if (!resumen.primer_avance || fecha < new Date(resumen.primer_avance)) {
    ...
    for (const resumen of lista) {
      ...
      if (resumen.total_intervenciones > 0) {
        const sumaAvance = resumen.intervenciones.reduce((s: number, i: IntervencionConAvances) => s + i.avance_obra_intervencion, 0)
        resumen.avance_obra_promedio = sumaAvance / resumen.total_intervenciones
      }
  ...
  const centrosConAvances = useMemo(
    () => resumenPorCentroGestor.filter(c => c.intervenciones_con_avance > 0).map(c => c.nombre_centro_gestor),
