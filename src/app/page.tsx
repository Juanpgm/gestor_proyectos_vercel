...
function DashboardContent() {
  ...
  const [filteredBpinsFromContracts, setFilteredBpinsFromContracts] = useState<
    number[] | undefined
  >(undefined);
  ...
  const filteredActividades: Actividad[] = useMemo(() => {
    return actividadesState.actividades.filter((activity) => {
      ...
      if (
        ...
        filters.centroGestor.length > 0 &&
        relatedProject?.nombre_centro_gestor
      ) {
        if (!filters.centroGestor.includes(relatedProject.nombre_centro_gestor))
          return false;
      }
    ...
    return productosState.productos.filter((product) => {
      ...
      if (filters.estado !== "all") {
        if (activeTab === "products") {
          const productState =
            product.avance_producto === 0
              ? "no_iniciado"
              : product.avance_producto === 1
                ? "completado"
                : "en_proceso";
    ...
    return contratosState.data.contratos.filter((contrato) => {
      ...
      if (filters.estado !== "all") {
        const relatedProject = proyectos.find(
          (p) => String(p.bpin) === String(contrato.bpin),
        );
  ...
  const filteredActividadesMetrics = useMemo(() => {
    ...
    const activitiesWithoutDates = filteredActividades.filter(
      (a) => !a.fecha_inicio_actividad || !a.fecha_fin_actividad,
    ).length;

    const activitiesWithDates = filteredActividades.filter(
      (a) => a.fecha_inicio_actividad && a.fecha_fin_actividad,
    );

    const completedActivities = activitiesWithDates.filter(
      (a) => a.avance_actividad === 1,
    ).length;
    const inProgressActivities = activitiesWithDates.filter(
      (a) => a.avance_actividad > 0 && a.avance_actividad < 1,
    ).length;
    const notStartedActivities = activitiesWithDates.filter(
      (a) => a.avance_actividad === 0,
    ).length;
    const averageProgress =
      filteredActividades.reduce((sum, a) => sum + a.avance_actividad, 0) /
  ...
  const filteredProductosMetrics = useMemo(() => {
    ...
    const completedProducts = filteredProductos.filter(
      (p) => p.avance_producto >= p.cantidad_programada_producto,
    ).length;
    const inProgressProducts = filteredProductos.filter(
      (p) =>
        p.avance_producto > 0 &&
        p.avance_producto < p.cantidad_programada_producto,
    ).length;
    const notStartedProducts = filteredProductos.filter(
      (p) => p.avance_producto === 0,
    ).length;

    const averageProgress =
      filteredProductos.reduce((sum, p) => {
        const progress =
          p.cantidad_programada_producto > 0
            ? p.avance_producto / p.cantidad_programada_producto
            : 0;
        return sum + Math.min(progress, 1);
      }, 0) / filteredProductos.length;

    const productsByType = filteredProductos.reduce(
      (acc, producto) => {
  ...
  const renderContent = () => {
        ...
        <div className="flex items-center justify-center h-64">
          <div className="text-center space-y-3">
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400 tabular-nums">
              ...
              {activeTab === "emprestito" && "Cargando empréstito…"}
    ...
    if (hasError) {
      const errorMessage =
        actividadesState.error ||
        productosState.error ||
        emprestitoState.error ||
        contratosState.error ||
        procesosState.error;
      return (
        <div className="flex items-center justify-center h-64">
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 xl:grid-cols-4 gap-6">
              <div className="xl:col-span-4 order-1">
                <div className="bg-white dark:bg-gray-900 rounded-md border border-gray-200 dark:border-gray-700 flex flex-col">
                  <div
                    className="overflow-hidden"
                    data-tour-id="section-projects-table"
                  >
          ...
          <div className="space-y-6">
            <div data-tour-id="section-contracts-stats">
              <ContratosStats
                ...
                totalValorContratos={
                  filteredContratosMetrics.totalValorContratos
                }
                ...
                valorPendienteEjecucion={
                  filteredContratosMetrics.valorPendienteEjecucion
                }
                contratosLiquidados={
                  filteredContratosMetrics.contratosLiquidados
                }
                contratosModificados={
                  filteredContratosMetrics.contratosModificados
                }
                contratosConPagoAdelantado={
                  filteredContratosMetrics.contratosConPagoAdelantado
                }
          ...
          <div className="space-y-8">
            <div data-tour-id="section-activities-stats">
              <ActividadesStats
                ...
                completedActivities={
                  filteredActividadesMetrics.completedActivities
                }
                inProgressActivities={
                  filteredActividadesMetrics.inProgressActivities
                }
                notStartedActivities={
                  filteredActividadesMetrics.notStartedActivities
                }
                activitiesWithoutDates={
                  filteredActividadesMetrics.activitiesWithoutDates
                }
            ...
            <div data-tour-id="section-procesos-stats">
              <ProcesosStats
                ...
                promedioVisualizaciones={
                  procesosMetrics.promedioVisualizaciones
                }
                promedioProveedoresInteres={
                  procesosMetrics.promedioProveedoresInteres
                }
