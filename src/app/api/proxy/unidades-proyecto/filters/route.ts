...
const normalizeFilterResponse = (payload: any): Record<string, unknown> => {
  ...
  if (
    !Array.isArray(normalized.comunas) &&
    Array.isArray(normalized.comunas_corregimientos)
  ) {
    normalized.comunas = normalized.comunas_corregimientos;
  }

  if (
    !Array.isArray(normalized.centros_gestores) &&
    Array.isArray(normalized.nombre_centro_gestor)
  ) {
