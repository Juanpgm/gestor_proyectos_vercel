...
export default function SolicitudesPendientesEmprestitoTab({
  onRefresh,
}: Props) {
  ...
  const cargarSolicitudes = useCallback(async () => {
    ...
    try {
      ...
      setSolicitudes(
        data.filter(
          (s) => !s.estado_decision || s.estado_decision === "pendiente",
        ),
      );
    } catch (err: any) {
  ...
  const handleAprobar = async (sol: SolicitudCambioEmprestito) => {
    ...
    try {
      await aprobarSolicitudEmprestito(
        sol.id,
        sol.tipo,
        sol.campos_modificados,
      );
      setToast({
        type: "success",
        message: `Solicitud ${sol.referencia} aprobada exitosamente`,
      });
      ...
      setToast({
        type: "error",
        message: err.message || "Error al aprobar solicitud",
      });
    } finally {
  ...
  const handleRechazar = async (sol: SolicitudCambioEmprestito) => {
    ...
    try {
      ...
      setToast({
        type: "success",
        message: `Solicitud ${sol.referencia} rechazada`,
      });
      setSolicitudes((prev) => prev.filter((s) => s.id !== sol.id));
      onRefresh?.();
    } catch (err: any) {
      setToast({
        type: "error",
        message: err.message || "Error al rechazar solicitud",
      });
    } finally {
      setProcessingId(null);
    }
        ...
        <div className="flex gap-2">
          {[
            ...
            >
              {t === "todos"
                ? "Todos"
                : EMPRESTITO_ENTITY_LABELS[
                    t as keyof typeof EMPRESTITO_ENTITY_LABELS
                  ] || t}
            </button>
          ))}
        </div>
                  ...
                  >
                    <div className="flex items-center gap-3 text-left">
                      ...
                      <div>
                        <div className="flex items-center gap-2">
                          <span
                            className={`inline-flex px-2 py-0.5 text-xs font-semibold rounded-full ${tipoBadge(sol.tipo)}`}
                          >
                        ...
                        <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
                          {sol.nombre_centro_gestor} · {sol.solicitado_por} ·{" "}
                        ...
                        <div className="px-4 py-3 space-y-3">
                          ...
                          <div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                              {Object.entries(sol.campos_modificados || {}).map(
                                ([field, change]) => (
                                  <div
                                    key={field}
