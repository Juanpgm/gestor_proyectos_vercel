...
const ConveniosTable: React.FC = () => {
  ...
  const [convenioToEdit, setConvenioToEdit] =
    useState<ConvenioTransferencia | null>(null);
  const [editingConvenio, setEditingConvenio] =
    useState<ConvenioTransferencia | null>(null);
  const [convenioToDelete, setConvenioToDelete] =
    useState<ConvenioTransferencia | null>(null);
  ...
  const handleUpdateConvenio = async (docId: string, formData: any) => {
    try {
      ...
      const response = await proxyFetch(
        "/api/proxy/emprestito/modificar-convenio-transferencia",
        {
          method: "PUT",
          headers: {
      ...
      if (!response.ok) {
        throw new Error(
          result.error || result.detail || "Error al actualizar el convenio",
        );
      }
      ...
      setConvenios((prev) =>
        prev.map((c) => {
          ...
          if (
            cId === docId ||
            c.referencia_contrato === formData.referencia_contrato
          ) {
            return { ...c, ...formData };
          }
          return c;
  ...
  const handleDeleteConvenio = async (convenio: ConvenioTransferencia) => {
    ...
    try {
      const referenciaContrato = String(
        convenio.referencia_contrato || "",
      ).trim();

      if (!referenciaContrato) {
        throw new Error(
          "No se encontró referencia_contrato para eliminar este convenio",
        );
      }

      const response = await proxyFetch(
        `/api/proxy/emprestito/eliminar-convenio-transferencia/${encodeURIComponent(referenciaContrato)}`,
        {
          method: "DELETE",
        },
      );
      ...
      setConvenios((prev) =>
        prev.filter(
          (item) =>
            item.id !== convenio.id &&
            item.referencia_contrato !== referenciaContrato,
        ),
      );
      await fetchConvenios(false);
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      totalValorContratos: allConvenios.reduce(
        (sum, c) => sum + parseNumeric(c.valor_contrato),
        0,
      ),
      totalValorConvenios: allConvenios.reduce(
        (sum, c) => sum + parseNumeric(c.valor_convenio),
        0,
      ),
      centrosGestores: new Set(
        allConvenios.map((c) => c.nombre_centro_gestor).filter(Boolean),
      ).size,
      bancos: new Set(allConvenios.map((c) => c.banco).filter(Boolean)).size,
      activos: allConvenios.filter((c) => c.estado_contrato === "En ejecución")
        .length,
    };
  ...
  return (
    <div className="space-y-6">
          ...
          {[
            {
              ...
              value: `${stats.filteredCount}${stats.filteredCount !== stats.totalConvenios ? ` / ${stats.totalConvenios}` : ""}`,
              icon: Handshake,
              color: "indigo",
            },
            {
              label: "Valor Total Contratos",
              value: formatCompactCurrency(stats.totalValorContratos),
              icon: DollarSign,
              color: "green",
            },
            {
              label: "Valor Total Convenios",
              value: formatCompactCurrency(stats.totalValorConvenios),
              icon: TrendingUp,
              color: "blue",
            },
            {
              label: "Centros Gestores",
              value: stats.centrosGestores,
              icon: Building,
              color: "purple",
            },
            {
              ...
              {showColumnSelector && (
                ...
                >
                  ...
                  <div className="mt-3 pt-3 border-t border-gray-200 dark:border-gray-700 flex justify-between text-xs text-gray-500 dark:text-gray-400">
                    <span>
                      {visibleColumns.size} de {columns.length} columnas
                      visibles
                    </span>
                    <button
                    ...
                    .map((column) => (
                      ...
                      >
                        <span
                          ...
                          title={String(convenio[column.key] || "")}
                        >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-4">
          <div className="w-full max-w-md rounded-lg bg-white dark:bg-gray-800 shadow-xl border border-gray-200 dark:border-gray-700 p-6">
            ...
            <p className="text-sm text-gray-600 dark:text-gray-300 mb-6">
              Â¿Está seguro que desea eliminar el convenio{" "}
              <span className="font-semibold">
                {convenioToDelete.referencia_contrato || "-"}
              </span>
      ...
      <ModificarConvenioModal
        ...
        onClose={() => {
          console.log("ðŸšª Cerrando modal de convenio");
          setShowModificarModal(false);
          setConvenioToEdit(null);
        }}
        onSuccess={async (updatedData?: any) => {
          console.log("âœ… OnSuccess de convenio llamado");
          ...
          if (updatedData && updatedData.referencia_contrato) {
            console.log(
              "ðŸ”„ Actualización optimista de convenio:",
              updatedData,
            );
