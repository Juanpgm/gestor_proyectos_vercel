...
import { modificarUnidadProyecto } from "@/services/unidades-proyecto.service";
import {
...
const EditarInfoUPModal: React.FC<EditarInfoUPModalProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!effectiveAccess.canViewAll) {
      const itemNorm = normalizeCentroGestor(item.nombre_centro_gestor);
      const userNorm = normalizeCentroGestor(effectiveAccess.userCentroGestor);
      if (itemNorm && userNorm && itemNorm !== userNorm) {
        setError("No tienes permiso para editar una UP de otro centro gestor.");
        return;
      }
    }
  ...
  return (
    ...
    >
      <motion.div
        ...
        role="dialog"
        aria-modal="true"
        aria-labelledby="editar-info-up-title"
      >
        {/* Header */}
        <div className="bg-gradient-to-r from-blue-600 to-indigo-600 dark:from-blue-700 dark:to-indigo-700 px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2
                id="editar-info-up-title"
                className="text-lg font-bold text-white"
              >
                Editar Información UP
              </h2>
            ...
            <div className="relative">
              <input
                ...
                onChange={(e) =>
                  setFormData((prev) => ({
                    ...prev,
                    avance_obra: parseFloat(e.target.value) || 0,
                  }))
                }
            ...
            <div className="mt-2 w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2">
              <div
                className={`h-2 rounded-full transition-all ${
                  formData.avance_obra >= 70
                    ? "bg-green-500"
                    : formData.avance_obra >= 40
                      ? "bg-yellow-500"
                      : "bg-red-500"
                }`}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input
                type="date"
                value={
                  formData.fecha_fin ? formData.fecha_fin.split("T")[0] : ""
                }
                onChange={(e) =>
                  setFormData((prev) => ({
                    ...prev,
        ...
        <div className="px-6 py-4 bg-gray-50 dark:bg-gray-900/50 border-t border-gray-200 dark:border-gray-700 flex items-center justify-between">
          <p className="text-xs text-gray-400 dark:text-gray-500">
            Los cambios se guardan localmente hasta la integración con el
            backend
          </p>
          <div className="flex items-center gap-3">
            <button
