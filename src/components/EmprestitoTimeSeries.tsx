...
const EmprestitoTimeSeries: React.FC<EmprestitoTimeSeriesProps> = ({
  ...
  const [selectedBancos, setSelectedBancos] = React.useState<Set<string>>(
    new Set(),
  );
  const [isInitialized, setIsInitialized] = React.useState(false);

  // Estado para filtro de meses (permite múltiples selecciones)
  const [selectedMeses, setSelectedMeses] = React.useState<Set<string>>(
    new Set(),
  );
    ...
    const fetchData = async () => {
      try {
        ...
        const response = await proxyFetch(
          "/api/proxy/emprestito/flujo-caja/all",
        );
        ...
        if (result.success && result.data) {
          ...
          const filteredData = filterByCentroGestor(
            result.data,
            centroGestorAccess,
            ["responsable", "organismo", "centro_gestor"],
          );
  ...
  React.useEffect(() => {
    if (mesesDisponibles.length > 0 && selectedMeses.size === 0) {
      setSelectedMeses(
        new Set([mesesDisponibles[mesesDisponibles.length - 1]]),
      );
    }
  ...
  const timeSeriesData = React.useMemo(() => {
    ...
    const mesesUnicos = Array.from(new Set(data.map((r) => r.mes))).sort(
      (a, b) => {
    ...
    const processedData = mesesUnicos.map((mes) => {
      ...
      bancosToShow.forEach((banco) => {
        const bancoTotal = data
          ...
          .reduce(
            (sum, row) => sum + (row.desembolso_real || row.desembolso || 0),
            0,
          );
  ...
  const totalesPorBanco = React.useMemo(() => {
    ...
    const bancosAgrupados: Record<
      string,
      { planeado: number; real: number; proyectos: Set<string> }
    > = {};

    data.forEach((record) => {
      if (!bancosAgrupados[record.banco]) {
        bancosAgrupados[record.banco] = {
          planeado: 0,
          real: 0,
          proyectos: new Set(),
        };
    ...
    return Object.entries(bancosAgrupados)
      .map(([banco, values]) => ({
        ...
        cumplimiento:
          values.planeado > 0 ? (values.real / values.planeado) * 100 : 0,
      }))
  ...
  const analisisOrganismos = React.useMemo(() => {
    ...
    const organismosAgrupados: Record<
      string,
      { planeado: number; real: number; proyectos: Set<string> }
    > = {};
  ...
  const getColorIntensity = (
    ...
    const lightColor = [
      baseColor[0] + (255 - baseColor[0]) * 0.6,
      baseColor[1] + (255 - baseColor[1]) * 0.6,
      baseColor[2] + (255 - baseColor[2]) * 0.6,
    ];
    const r = Math.round(
      lightColor[0] + (baseColor[0] - lightColor[0]) * intensity,
    );
    const g = Math.round(
      lightColor[1] + (baseColor[1] - lightColor[1]) * intensity,
    );
    const b = Math.round(
      lightColor[2] + (baseColor[2] - lightColor[2]) * intensity,
    );
    return `rgb(${r}, ${g}, ${b})`;
  };
  ...
  if (error) {
    return (
      <motion.div
        ...
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.4 }}
        className={`bg-white dark:bg-gray-800 rounded-md p-6 shadow-none border border-red-200 dark:border-red-700 ${className}`}
      >
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-2">
        ...
        >
          ...
          <div className="text-xl font-bold">
            {formatNumber(
              data.reduce((sum, r) => sum + (r.desembolso || 0), 0),
              "currency",
            )}
          </div>
        </motion.div>
        ...
        >
          ...
          <div className="text-xl font-bold">
            {formatNumber(
              data.reduce((sum, r) => sum + (r.desembolso_real || 0), 0),
              "currency",
            )}
          </div>
        </motion.div>
        ...
        >
          ...
          <div className="text-xl font-bold">
            {
              bancos.filter((b) =>
                totalesPorBanco.find((t) => t.banco === b && t.planeado > 0),
              ).length
            }
          </div>
        </motion.div>
          ...
          <div className="text-xl font-bold">
            {(() => {
              const totalPlaneado = data.reduce(
                (sum, r) => sum + (r.desembolso || 0),
                0,
              );
              const totalReal = data.reduce(
                (sum, r) => sum + (r.desembolso_real || 0),
                0,
              );
              return totalPlaneado > 0
      ...
      <motion.div
        ...
        className="bg-white dark:bg-gray-800 rounded-md p-4 shadow-none border border-gray-200 dark:border-gray-700"
      >
        ...
        {bancos.length > 0 && (
          <div className="mb-4">
            ...
            <div className="flex flex-wrap gap-2">
              {bancos
                .filter((b) => totalesPorBanco.find((t) => t.banco === b))
                .map((banco) => (
                  <label
                    key={banco}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        >
          <div className="flex items-center gap-2 mb-3">
            <div
              className={`p-1.5 rounded-lg bg-gradient-to-br ${CATEGORIES.emprestito.gradient}`}
            >
              ...
              <PieChart>
                ...
                <Legend
                  ...
                  formatter={(value, entry: any) => {
                    ...
                    return item
                      ? `${value} (${((item.planeado / totalesPorBanco.reduce((s, b) => s + b.planeado, 0)) * 100).toFixed(1)}%)`
                      : value;
                  }}
                />
              </PieChart>
        ...
        <div className="h-96">
          {(() => {
            ...
            filteredData.forEach((record) => {
              const org = record.organismo || "Sin clasificar";
              porOrganismo[org] =
                (porOrganismo[org] || 0) + (record.desembolso || 0);
            });
      ...
      {/* Tabla Principal: Organismo â†’ Proyectos â†’ Desembolso */}
      <motion.div
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
      ...
      >
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            ...
            <p className="text-sm text-gray-600 dark:text-gray-400">
              Organismo â†’ Proyectos â†’ Desembolso
            </p>
          </div>
        </div>
            ...
            <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
              {(() => {
                const filteredData = data.filter(
                  (record) =>
                    (!selectedBancos.size ||
                      selectedBancos.has(record.banco)) &&
                    selectedMeses.has(record.mes) &&
                    (record.desembolso || 0) > 0,
                );
                ...
                organismosOrdenados.forEach(({ org, records, total }) => {
                  ...
                  const proyectosOrdenados = records.sort(
                    (a, b) => (b.desembolso || 0) - (a.desembolso || 0),
                  );
