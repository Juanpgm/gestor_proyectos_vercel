...
const Sidebar: React.FC<SidebarProps> = ({
  ...
  const visibleManagementMenuItems = canAccessFullManagementSidebar
    ? managementMenuItems
    : isAdminCentroGestor
      ? managementMenuItems.filter(
          (item) =>
            item.id === "gestionar-unidades-proyecto" ||
            item.id === "gestionar-emprestito",
        )
      : [];
  ...
  useEffect(() => {
    console.log("🔍 Sidebar - Validación de módulo Gestionar Usuarios:", {
      ...
      isAdminCentroGestor,
      totalMenuItems: shouldShowUserManagement
        ? baseMenuItems.length + 1
