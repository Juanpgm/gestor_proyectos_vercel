...
export interface BadgeProps {
  children: ReactNode
  color?:   'default' | 'blue' | 'green' | 'red' | 'orange' | 'violet' | 'teal' | 'amber' | 'gray'
  size?:    'sm' | 'md'
  dot?:     boolean
  className?: string
}

const colorClasses: Record<NonNullable<BadgeProps['color']>, { badge: string; dot: string }> = {
  default: { badge: 'bg-gray-100 text-gray-700 dark:bg-gray-700 dark:text-gray-300',      dot: 'bg-gray-400' },
  gray:    { badge: 'bg-gray-100 text-gray-700 dark:bg-gray-700 dark:text-gray-300',      dot: 'bg-gray-400' },
  ...
  teal:    { badge: 'bg-teal-100 text-teal-700 dark:bg-teal-900/40 dark:text-teal-300',   dot: 'bg-teal-500' },
...
export function Badge({ children, color = 'default', size = 'md', dot = false, className }: BadgeProps) {
  ...
  return (
    ...
    >
      {dot && (
        <span
          className={cn('shrink-0 rounded-full', dotColor, size === 'sm' ? 'w-1.5 h-1.5' : 'w-2 h-2')}
          aria-hidden="true"
        />
      )}
      {children}
