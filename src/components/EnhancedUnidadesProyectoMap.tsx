/**
 * Componente del mapa con controles de coloraciÃ³n
...
const BaseLayerLabels: React.FC<{
  ...
  const labels = data.features
    .map((feature: any, index: number) => {
      ...
      // Usar las propiedades correctas segÃºn el tipo de capa
      const name =
        layerType === "comunas"
...
const UnidadesProyectoMap: React.FC<UnidadesProyectoMapProps> = ({
  ...
  const getBaseLayerStyle = (feature: any) => {
    if (baseLayerColorMode === "monotone") {
      ...
      // FunciÃ³n de hash mejorada (DJB2) - solo usar el nombre para consistencia
      let hash = 5381;
      for (let i = 0; i < featureName.length; i++) {
        hash = (hash << 5) + hash + featureName.charCodeAt(i);
      }

      // Asegurar distribuciÃ³n uniforme en la paleta
      const colorIndex = Math.abs(hash) % colorPalette.length;
      const color = colorPalette[colorIndex];

      console.log(
        `Feature: ${featureName}, Hash: ${hash}, Index: ${colorIndex}, Color: ${color}`,
      );
  ...
  // Consolidar datos por UPID para coloraciÃ³n (estado, tipo, centro, avance, inversiÃ³n)
    ...
    switch (coloringType) {
      ...
      default: {
        ...
        const uniqueValues = Array.from(
          new Set(data.map((item) => String(item[field])).filter(Boolean)),
        );
        ...
        uniqueValues.forEach((value, index) => {
          ...
          valueCounts.set(
            value,
            data.filter((item) => String(item[field]) === value).length,
          );
        });
