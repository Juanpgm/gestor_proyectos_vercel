...
const EmprestitoFlujoCajaDashboard: React.FC<
  EmprestitoFlujoCajaDashboardProps
> = ({ className = "" }) => {
  const [flujoCajaData, setFlujoCajaData] = useState<FlujoCajaResponse | null>(
    null,
  );
  const [loading, setLoading] = useState(true);
  const [selectedBancos, setSelectedBancos] = useState<Set<string>>(new Set());
  const [selectedOrganismos, setSelectedOrganismos] = useState<Set<string>>(
    new Set(),
  );
    ...
    const fetchFlujoCaja = async () => {
      try {
        ...
        if (data.metadata) {
          setSelectedBancos(
            new Set(
              data.metadata.bancos.filter((b: string) => b && b !== "nan"),
            ),
          );
  ...
  const timeSeriesData = useMemo(() => {
    ...
    const filteredData = flujoCajaData.data.filter(
      (row) =>
  ...
  const bankAnalysis = useMemo(() => {
    ...
    const bankTotals = filteredData.reduce(
      (acc, row) => {
        if (!acc[row.banco]) {
          acc[row.banco] = {
            banco: row.banco,
  ...
  const organismoAnalysis = useMemo(() => {
    ...
    const organismoTotals = filteredData.reduce(
      (acc, row) => {
  ...
  const plannedVsRealData = useMemo(() => {
    ...
    const monthlyComparison = filteredData.reduce(
      (acc, row) => {
        const month = row.mes;
        if (!acc[month]) {
          acc[month] = {
  ...
  const totalReal =
    flujoCajaData?.data.reduce(
      (sum, row) => sum + (row.desembolso_real || 0),
      0,
    ) || 0;
    ...
    <div className={`space-y-6 ${className}`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        >
          ...
          <div className="text-2xl font-bold">
            {formatNumber(totalDesembolso, "currency")}
          </div>
        ...
        >
          ...
          <div className="text-2xl font-bold">
            {formatNumber(totalReal, "currency")}
          </div>
        ...
        >
          ...
          <div className="text-2xl font-bold">
            {flujoCajaData?.summary?.bancos_unicos || 0}
          </div>
        ...
        >
          ...
          <div className="text-2xl font-bold">
            {totalDesembolso > 0
              ? ((totalReal / totalDesembolso) * 100).toFixed(1)
              : 0}
            %
          </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-md p-6 shadow-none border border-gray-200 dark:border-gray-700">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                .map((banco) => (
                  ...
                  >
                    ...
                    <span
                      ...
                      style={{ color: BANCO_COLORS[banco] || "#6B7280" }}
                    >
                      {banco}
                    </span>
                  </label>
          ...
          <div>
            <label className="text-xs font-medium text-gray-600 dark:text-gray-400 mb-2 block">
              Organismos ({selectedOrganismos.size}/
              {flujoCajaData?.metadata?.organismos.length || 0})
            </label>
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            ...
            <p className="text-sm text-gray-600 dark:text-gray-400">
              Flujo de caja mensual por banco (barras) y acumulado total
              (lÃ­nea)
            </p>
          </div>
        </div>
            ...
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie
                  ...
                  outerRadius={100}
                  label={(entry) =>
                    `${entry.banco}: ${formatNumber(entry.total, "currency")}`
                  }
                ...
                >
                  {bankAnalysis.map((entry: any, index: number) => (
                    <Cell
                      key={`cell-${index}`}
                      fill={BANCO_COLORS[entry.banco] || "#6B7280"}
                    />
                  ))}
                </Pie>
                <Tooltip content={<CustomTooltip />} />
                  ...
                  <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2">
                    <div
                      ...
                      style={{
                        width: `${percentage}%`,
                        backgroundColor:
                          ORGANISMO_COLORS[index % ORGANISMO_COLORS.length],
                      }}
                    />
                  </div>
