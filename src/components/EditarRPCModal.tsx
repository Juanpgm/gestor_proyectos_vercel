...
import { X, AlertCircle, CheckCircle, Plus, Trash2, Edit2, FileText } from 'lucide-react'
...
const EditarRPCModal: React.FC<EditarRPCModalProps> = ({
  ...
  useEffect(() => {
    if (isOpen && rpcData) {
      setFormData({
        ...
        nombre_centro_gestor: rpcData.nombre_centro_gestor || contratoData?.nombre_centro_gestor || '',
        referencia_contrato: rpcData.referencia_contrato || contratoData?.referencia_contrato || ''
      })

      if (rpcData.cdp_asociados && Array.isArray(rpcData.cdp_asociados)) {
        setCdps(rpcData.cdp_asociados.map((cdp, index) => ({
      ...
      if (rpcData.programacion_pac && typeof rpcData.programacion_pac === 'object') {
        const pagos = Object.entries(rpcData.programacion_pac).map(([key, valor], index) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const numeroRpc = formData.numero_rpc?.trim() || rpcData?.numero_rpc?.trim()
      ...
      if (!response.ok) {
        throw new Error(result.error || `Error al actualizar RPC: ${response.status}`)
      }
      ...
      setError(err instanceof Error ? err.message : 'Error desconocido al actualizar RPC')
    } finally {
      setLoading(false)
    }
  }
          ...
          <div className="p-6 overflow-y-auto max-h-[calc(90vh-140px)]">
            ...
            {success && (
              <div className="mb-4 p-4 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg flex items-start space-x-3">
                ...
                <p className="text-sm text-green-800 dark:text-green-200">RPC actualizado exitosamente</p>
              </div>
            )}
                ...
                <div>
                  ...
                  <div className="p-4 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg">
                    ...
                    <p className="text-xs text-blue-600 dark:text-blue-300 mb-3">
                      Los documentos no se pueden modificar desde este formulario.
                    </p>
