...
import { MAP_PROGRESS_COLORS } from "@/lib/map-theme";
...
const MapMeasureTool: React.FC = () => {
  ...
  const redraw = useCallback(
    (pts: L.LatLng[], currentMode: MeasureMode) => {
      polylineRef.current?.remove();
      polylineRef.current = null;
      polygonRef.current?.remove();
      polygonRef.current = null;
      ...
      if (currentMode === "distance") {
        polylineRef.current = L.polyline(latLngs, {
          color: MAP_PROGRESS_COLORS.good,
          weight: 2,
          dashArray: "8, 6",
          opacity: 0.9,
        }).addTo(map);
    ...
    const onClick = (e: L.LeafletMouseEvent) => {
      ...
      setPoints((prev) => {
        ...
        const marker = L.circleMarker(latlng, {
          radius: 5,
          color: mode === "distance" ? MAP_PROGRESS_COLORS.good : "#7c3aed",
          fillColor: "#ffffff",
          fillOpacity: 1,
          weight: 2,
        }).addTo(map);
  ...
  return (
    <>
      ...
      <div ref={toolbarRef} className="absolute top-4 left-16 z-[1000]">
        <div className="flex items-start gap-2">
          {/* Mode buttons */}
          <div className="bg-white dark:bg-gray-800 rounded-md shadow-none border border-gray-200 dark:border-gray-700 flex items-center p-1 space-x-0.5">
            <button
          ...
          {mode !== "none" && points.length >= 2 && (
            <div className="bg-white dark:bg-gray-800 rounded-md shadow-none border border-gray-200 dark:border-gray-700 px-2.5 py-1.5 text-sm flex items-center space-x-2">
              {mode === "distance" && (
                <>
                  <Ruler
                    size={13}
                  ...
                  <div className="relative">
                    ...
                    {showUnitDropdown && (
                      <div className="absolute top-full left-0 mt-1 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-lg py-1 min-w-[56px] z-10">
                        {(Object.keys(AREA_UNIT_LABELS) as AreaUnit[]).map(
                          (u) => (
                            <button
                              key={u}
                              onClick={(e) => {
        ...
        {isDrawing && (
          <div className="mt-1 text-[10px] text-gray-500 dark:text-gray-400 bg-white/80 dark:bg-gray-800/80 rounded px-2 py-0.5 inline-block backdrop-blur-sm">
            Click para anadir puntos ·{" "}
            <Check size={10} strokeWidth={1.5} className="inline" /> o
            doble-click para confirmar · Esc para cancelar
          </div>
        )}
      </div>
    </>
