...
interface FilterState {
  ...
  periodos: string[]; // Cambiado de aÃ±o: string a periodos: string[] para mÃºltiples selecciones
}
...
export default function UnifiedFilters({
  ...
  const [fuenteFinanciamientoSearch, setFuenteFinanciamientoSearch] =
    useState("");
  const [centroGestorSearch, setCentroGestorSearch] = useState("");
  const [filtrosPersonalizadosSearch, setFiltrosPersonalizadosSearch] =
    useState("");
  ...
  // Nota: PerÃ­odos eliminados segÃºn requerimientos
  ...
  useEffect(() => {
    const handleGlobalClick = (event: Event) => {
      ...
      const isInSuggestions = suggestionsDropdownRef.current?.contains(
        target as Node,
      );

      if (!isInSearchInput && !isInSuggestions) {
        scheduleAutoHide(100); // Ocultar rÃ¡pido
      }
    };
    ...
    return () => {
      ...
      // Limpiar flag de supresiÃ³n
      suppressSuggestionsRef.current = false;
    };
  }, []);

  // Generar sugerencias de bÃºsqueda comprehensiva ADAPTADAS A LA SECCIÃ“N ACTIVA
  useEffect(() => {
    const searchTerm = safeFilters.search.trim().toLowerCase();

    // NUEVA VERIFICACIÃ“N: Si las sugerencias estÃ¡n suprimidas, no generar
    ...
    if (isNumericSearch) {
      // BÃºsqueda prioritaria por BPIN para valores numÃ©ricos
      ...
      // NUEVA LÃ“GICA: Priorizar segÃºn secciÃ³n activa

      // 1. SECCIÃ“N ACTIVIDADES: Priorizar actividades
      if (activeTab === "activities") {
        allProjects.forEach((projectData) => {
          ...
          actividades.forEach((actividad: any) => {
            ...
            if (
              nombreActividad.toLowerCase().includes(searchTerm) ||
      ...
      // 2. SECCIÃ“N PRODUCTOS: Priorizar productos
        ...
        allProjects.forEach((projectData) => {
          ...
          productos.forEach((producto: any) => {
            ...
            if (
              nombreProducto.toLowerCase().includes(searchTerm) ||
      ...
      // 3. BÃšSQUEDA EN PROYECTOS (siempre relevante)
      allProjects.forEach((projectData) => {
      ...
      // 4. BÃºsqueda en centros gestores (prioridad alta)
      centrosGestores.forEach((centro) => {
        if (
          centro.toLowerCase().includes(searchTerm) &&
      ...
      // 5. BÃºsqueda en ubicaciones (comunas/barrios) - prioridad alta
      ...
      // 6. BÃºsqueda en fuentes de financiamiento
      fuentesFinanciamiento.forEach((fuente) => {
        if (
          fuente.toLowerCase().includes(searchTerm) &&
      ...
      // 7. BÃºsqueda secundaria en otras secciones (solo si no es la secciÃ³n activa)
      if (activeTab !== "activities") {
        allProjects.forEach((projectData) => {
    ...
    // Eliminar duplicados y mostrar hasta 16 sugerencias mÃ¡ximo con mejor organizaciÃ³n
  ...
  useEffect(() => {
    // Si showSuggestions es false, asegurar que todo estÃ© limpio
    if (!showSuggestions) {
      setSelectedSuggestionIndex(-1);
      // No limpiar searchSuggestions aquÃ­ para evitar parpadeos
    }
  }, [showSuggestions, searchSuggestions.length]);

  // Effect adicional para cleanup agresivo - REMOVED: Causaba loops infinitos
  // useEffect(() => {
  //   // Si no hay sugerencias visibles pero el array no estÃ¡ vacÃ­o, limpiarlo
  //   if (!showSuggestions && searchSuggestions.length > 0) {
  //     console.log('ðŸ§¹ Limpieza agresiva de sugerencias')
  //     setTimeout(() => {
  //       if (!showSuggestions) { // Verificar nuevamente despuÃ©s del timeout
  //         setSearchSuggestions([])
  //       }
  ...
  // Effect para limpiar las sugerencias cuando no hay bÃºsqueda activa
  useEffect(() => {
  ...
  // FunciÃ³n simplificada para ocultar sugerencias
  ...
  // FunciÃ³n para forzar ocultamiento inmediato
  const forceHideSuggestions = () => {
    hideSuggestions();
    // Quitar foco del input si estÃ¡ activo
  ...
  // FunciÃ³n para programar auto-ocultamiento
  ...
  // Opciones de centro gestor cargadas dinÃ¡micamente desde JSON
  ...
  // Opciones de fuentes de financiamiento cargadas dinÃ¡micamente desde JSON
  ...
  // FunciÃ³n para obtener veredas dinÃ¡micamente segÃºn corregimientos seleccionados
  const getVeredasForCorregimientos = (
    selectedCorregimientos: string[] | undefined,
  ) => {
    if (!selectedCorregimientos || selectedCorregimientos.length === 0)
      return [];
    return getVeredasPorCorregimientos(selectedCorregimientos);
  };

  // Mapeo de filtros personalizados a subfiltros (jerÃ¡rquico)
  ...
  const getSubfiltrosForFiltros = (selectedFiltros: string[] | undefined) => {
    ...
    selectedFiltros.forEach((filtro) => {
      const subfiltros =
        filtroSubfiltroMap[filtro as keyof typeof filtroSubfiltroMap] || [];
  ...
  // NUEVA funciÃ³n simplificada para forzar ocultamiento
  const handleSuggestionSelect = (suggestion: {
    value: string;
    type: string;
    label: string;
  }) => {
    // NUEVA LÃ“GICA: Generar filtros especÃ­ficos basados en el tipo de sugerencia
    switch (suggestion.type) {
      ...
      case "Producto":
      case "Datos":
        // Para estos tipos, mantener como bÃºsqueda de texto
        updateFilters({ search: suggestion.value });
        // OCULTAR DESPUÃ‰S de actualizar filtros para tipos de bÃºsqueda de texto
        setTimeout(() => {
      ...
      case "Centro Gestor":
        ...
        if (!currentCentros.includes(suggestion.value)) {
          updateFilters({
            ...
            search: "", // Limpiar bÃºsqueda
          });
        }
        // Ocultar inmediatamente para filtros especÃ­ficos
        forceHideSuggestions();
        break;
      ...
      default:
        // Para tipos no reconocidos, usar como bÃºsqueda de texto
        updateFilters({ search: suggestion.value });
        // OCULTAR DESPUÃ‰S de actualizar filtros para tipos de bÃºsqueda de texto
        setTimeout(() => {
          forceHideSuggestions();
  ...
  const resetFilters = () => {
    if (onFiltersChange) {
      ...
      // ocultar sugerencias de bÃºsqueda
      setShowSuggestions(false);
    }
  };

  // Generar placeholder dinÃ¡mico segÃºn secciÃ³n activa
  ...
  const getActiveFiltersCount = () => {
    ...
    if (safeFilters.centroGestor && safeFilters.centroGestor.length > 0)
      count++;
    if (safeFilters.comunas && safeFilters.comunas.length > 0) count++;
    if (safeFilters.barrios && safeFilters.barrios.length > 0) count++;
    if (safeFilters.corregimientos && safeFilters.corregimientos.length > 0)
      count++;
    if (safeFilters.veredas && safeFilters.veredas.length > 0) count++;
    if (
      safeFilters.fuentesFinanciamiento &&
      safeFilters.fuentesFinanciamiento.length > 0
    )
      count++;
  ...
  const removeFilter = (filterType: string, value?: string) => {
    switch (filterType) {
      ...
      case "centroGestor":
        if (value) {
          updateFilters({
            centroGestor: safeFilters.centroGestor.filter((c) => c !== value),
          });
        } else {
      ...
      case "comunas":
        if (value) {
          ...
          if (updatedComunas.length === 0) {
            ...
            const allowedBarrios = new Set(
              getBarriosPorComunas(updatedComunas),
            );
            const filteredBarrios = (safeFilters.barrios || []).filter((b) =>
              allowedBarrios.has(b),
            );
            updateFilters({
      ...
      case "barrios":
        if (value) {
          updateFilters({
            barrios: safeFilters.barrios.filter((b) => b !== value),
          });
        } else {
      ...
      case "corregimientos":
        if (value) {
          const updatedCorregimientos = safeFilters.corregimientos.filter(
            (c) => c !== value,
          );
          if (updatedCorregimientos.length === 0) {
            updateFilters({ corregimientos: [], veredas: [] });
          } else {
            const allowedVeredas = new Set(
              getVeredasForCorregimientos(updatedCorregimientos),
            );
            const filteredVeredas = (safeFilters.veredas || []).filter((v) =>
              allowedVeredas.has(v),
            );
            updateFilters({
      ...
      case "veredas":
        if (value) {
          updateFilters({
            veredas: safeFilters.veredas.filter((v) => v !== value),
          });
        } else {
      ...
      case "fuentesFinanciamiento":
        if (value) {
          updateFilters({
            fuentesFinanciamiento: safeFilters.fuentesFinanciamiento.filter(
              (f) => f !== value,
            ),
          });
        } else {
      ...
      case "filtrosPersonalizados":
        if (value) {
          updateFilters({
            filtrosPersonalizados: safeFilters.filtrosPersonalizados.filter(
              (f) => f !== value,
            ),
          });
        } else {
      ...
      case "periodos":
        if (value) {
          updateFilters({
            periodos: safeFilters.periodos.filter((p) => p !== value),
          });
        } else {
  ...
  // Funciones para manejar las dependencias jerÃ¡rquicas
  ...
  const handleComunaChange = (comuna: string, checked: boolean) => {
    ...
    if (checked) {
      ...
      // Filtrar barrios que ya no son vÃ¡lidos
  ...
  const handleCorregimientoChange = (
    ...
    if (checked) {
      ...
      const updatedCorregimientos = currentCorregimientos.filter(
        (c) => c !== corregimiento,
      );

      // Filtrar veredas que ya no son vÃ¡lidas
      const validVeredas = getVeredasForCorregimientos(updatedCorregimientos);
      const filteredVeredas = currentVeredas.filter((v) =>
        validVeredas.includes(v),
      );
  ...
  const handleFuenteFinanciamientoChange = (
    fuente: string,
    checked: boolean,
  ) => {
    const currentFuentes = safeFilters.fuentesFinanciamiento || [];
  ...
  const handleFiltrosPersonalizadosChange = (
    filtro: string,
    checked: boolean,
  ) => {
  ...
  // Listas a mostrar filtradas segÃºn las selecciones padre
  ...
  const displayedVeredas = getVeredasForCorregimientos(
    safeFilters.corregimientos,
  );

  // Apply search filtering for dropdown lists
  const filteredComunas = useMemo(() => {
    return comunasOptions.filter((c: string) =>
      c.toLowerCase().includes(comunasSearch.toLowerCase()),
    );
  }, [comunasOptions, comunasSearch]);

  const filteredBarrios = displayedBarrios.filter((b: string) =>
    b.toLowerCase().includes(barriosSearch.toLowerCase()),
  );
  const filteredCorregimientos = corregimientosOptions.filter((c: string) =>
    c.toLowerCase().includes(corregimientosSearch.toLowerCase()),
  );
  const filteredVeredas = displayedVeredas.filter((v: string) =>
    v.toLowerCase().includes(veredasSearch.toLowerCase()),
  );
  const filteredFuentesFinanciamiento = fuentesFinanciamientoOptions.filter(
    (f: string) =>
      f.toLowerCase().includes(fuenteFinanciamientoSearch.toLowerCase()),
  );
  const filteredCentroGestor = centroGestorOptions.filter((c: string) =>
    c.toLowerCase().includes(centroGestorSearch.toLowerCase()),
  );
  const filteredFiltrosPersonalizados = filtrosPersonalizadosOptions.filter(
    (f: string) =>
      f.toLowerCase().includes(filtrosPersonalizadosSearch.toLowerCase()),
  );

  return (
    <section
      className={`bg-white dark:bg-gray-800 rounded-md shadow-none border border-gray-100 dark:border-gray-700 transition-colors duration-300 ${className}`}
    >
      {/* Header */}
        ...
        <section className="flex items-center justify-between">
          ...
          <nav className="flex items-center space-x-2">
            <button
              ...
              title="Mostrar información de depuración en consola (F12)"
            >
              Debug
            </button>
            <button
      ...
      >
        <main className="p-4 space-y-4 overflow-visible">
            ...
            <section className="flex items-center space-x-3">
              ...
              <section className="flex-1 relative" ref={searchDropdownRef}>
                ...
                {showSuggestions && searchSuggestions.length > 0 && (
                  <section
                    ...
                    onMouseEnter={() => {
                      // Cancelar auto-ocultamiento cuando el mouse estÃ¡ sobre las sugerencias
                      if (autoHideTimerRef.current) {
                        clearTimeout(autoHideTimerRef.current);
                        autoHideTimerRef.current = null;
                      }
                      ...
                      <header className="flex items-center justify-between text-xs font-semibold text-gray-500 dark:text-gray-400 px-2 py-1 uppercase tracking-wide border-b border-gray-200 dark:border-gray-600 mb-2">
                        <span>
                          {/^\d+$/.test(safeFilters.search.trim())
                            ? "Resultados BPIN (Optimizado)"
                            : "Filtros Sugeridos - Click para Aplicar"}
                        </span>
                        <button
                          onClick={(e) => {
                            e.preventDefault();
                      ...
                      {searchSuggestions.map((suggestion, index) => (
                        <button
                          ...
                          onClick={(e) => {
                            ...
                            // Ejecutar selecciÃ³n inmediatamente
                          ...
                          <section className="flex-shrink-0">
                            <span
                              className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${
                                suggestion.type === "BPIN"
                                  ? "bg-cyan-100 text-cyan-800 dark:bg-cyan-900/20 dark:text-cyan-300 border border-cyan-300 font-semibold"
                                  : suggestion.type === "Centro Gestor"
                                    ? "bg-teal-100 text-teal-800 dark:bg-teal-900/20 dark:text-teal-300 border border-teal-300"
                                    : suggestion.type === "Comuna"
                                      ? "bg-blue-100 text-blue-800 dark:bg-blue-900/20 dark:text-blue-300 border border-blue-300"
                                      : suggestion.type === "Barrio"
                                        ? "bg-green-100 text-green-800 dark:bg-green-900/20 dark:text-green-300 border border-green-300"
                                        : suggestion.type === "Fuente"
                                          ? "bg-purple-100 text-purple-800 dark:bg-purple-900/20 dark:text-purple-300 border border-purple-300"
                                          : suggestion.type === "Proyecto"
                                            ? "bg-pink-100 text-pink-800 dark:bg-pink-900/20 dark:text-pink-300 border border-pink-300"
                                            : suggestion.type === "Actividad"
                                              ...
                                              : suggestion.type === "Producto"
                                                ? "bg-yellow-100 text-yellow-800 dark:bg-yellow-900/20 dark:text-yellow-300 border border-yellow-600"
              ...
              {safeFilters.search && (
                <section className="flex items-center space-x-2">
                  ...
                  {/* BOTÃ“N DE EMERGENCIA para cerrar sugerencias */}
                  {showSuggestions && (
                    <button
                      onClick={(e) => {
                        e.preventDefault();
            ...
            {safeFilters.search && (
              <section className="mt-2 flex items-center justify-between">
                <p className="text-xs text-blue-600 dark:text-blue-400">
                  Buscando: &ldquo;
                  <span className="font-medium">{safeFilters.search}</span>
                  &rdquo;
                  {/^\d+$/.test(safeFilters.search.trim()) && (
                    <span className="ml-2 inline-flex items-center px-2 py-1 rounded-full text-xs bg-cyan-100 text-cyan-800 dark:bg-cyan-900/20 dark:text-cyan-300 border border-cyan-300">
                      ðŸŽ¯ BÃºsqueda optimizada por BPIN
                    </span>
                  )}
                </p>
              </section>
          ...
          {/* Centro Gestor y AÃ±o */}
          <section className="flex gap-3">
            {/* Centro Gestor - 80% del espacio */}
            <section
              className="relative flex-1 w-4/5"
              data-dropdown="centro_gestor"
            >
              <button
                onClick={(e) => {
                  e.preventDefault();
              ...
              >
                <span className="text-sm font-medium text-gray-700 dark:text-gray-300 truncate">
                  Centro Gestor{" "}
                  {safeFilters.centroGestor?.length > 0 &&
                    `(${safeFilters.centroGestor.length})`}
                </span>
                <ChevronDown
                  className={`w-4 h-4 text-gray-600 dark:text-gray-400 transition-transform duration-200 ml-2 flex-shrink-0 ${openDropdowns.centro_gestor ? "rotate-180" : ""}`}
                />
              </button>
                      ...
                      {filteredCentroGestor.map((centro) => (
                        ...
                        >
                          <input
                            ...
                            checked={
                              safeFilters.centroGestor?.includes(centro) ||
                              false
                            }
                            onChange={(e) => {
                              handleCentroGestorChange(
                                centro,
                                e.target.checked,
                              );
                            }}
              ...
              >
                <span className="text-sm font-medium text-gray-700 dark:text-gray-300 truncate">
                  ...
                  {safeFilters.periodos && safeFilters.periodos.length > 0
                    ? `(${safeFilters.periodos.length})`
                    : ""}
                </span>
                <ChevronDown
                  className={`w-4 h-4 text-gray-600 dark:text-gray-400 transition-transform duration-200 ml-1 flex-shrink-0 ${openDropdowns.periodo ? "rotate-180" : ""}`}
                />
              </button>
                ...
                <section className="absolute top-full left-0 right-0 mt-1 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 rounded-lg shadow-xl z-[9999] max-h-60 overflow-y-auto">
                  <section className="p-2">
                    <section className="space-y-1">
                      ...
                      {["2024", "2025", "2026", "2027"].map((año) => (
                        ...
                        >
                          <input
                            ...
                            checked={
                              safeFilters.periodos?.includes(año) || false
                            }
                            onChange={(e) =>
                              handlePeriodoChange(año, e.target.checked)
                            }
                          />
                          <span className="text-sm text-gray-700 dark:text-gray-300">
                            {año}
                      ...
                      {["2024-2027", "2020-2023", "2016-2019"].map(
                        (periodo) => (
                          <label
                            key={periodo}
                            className="flex items-center space-x-2 px-2 py-1 hover:bg-gray-50 dark:hover:bg-gray-700 rounded cursor-pointer"
                          >
                            <input
                              ...
                              checked={
                                safeFilters.periodos?.includes(periodo) || false
                              }
                              onChange={(e) =>
                                handlePeriodoChange(periodo, e.target.checked)
                              }
                            />
                            <span className="text-sm text-gray-700 dark:text-gray-300">
                              {periodo}
                      ...
                      {safeFilters.periodos &&
                        safeFilters.periodos.length > 0 && (
                          <button
                            onClick={() => {
                              updateFilters({ periodos: [] });
                              setOpenDropdowns((prev) => ({
          ...
          <section
            ref={dropdownRef}
            className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-5 gap-1 relative"
            data-dropdown="filters-container"
          >
            ...
            <article className="relative">
              ...
              >
                <span className="text-xs font-medium text-pink-700 dark:text-pink-300 truncate">
                  Filtros
                </span>
                <ChevronDown
                  className={`w-3 h-3 text-pink-600 transition-transform duration-200 ml-1 flex-shrink-0 ${openDropdowns.filtros_personalizados ? "rotate-180" : ""}`}
                />
              </button>
                ...
                <section className="absolute top-full left-0 right-0 mt-1 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 rounded-lg shadow-xl z-[9999] max-h-80 overflow-y-auto">
                  <section className="p-3">
                    <input
                      ...
                      onChange={(e) =>
                        setFiltrosPersonalizadosSearch(e.target.value)
                      }
                    ...
                    <section className="space-y-1 max-h-60 overflow-y-auto">
                      {filteredFiltrosPersonalizados.map((filtro: string) => (
                        <label
                          key={filtro}
                          className="flex items-center space-x-2 p-1 hover:bg-pink-50 dark:hover:bg-pink-900/20 rounded cursor-pointer"
                        >
                          <input
                            type="checkbox"
                            checked={
                              safeFilters.filtrosPersonalizados?.includes(
                                filtro,
                              ) || false
                            }
                            onChange={(e) =>
                              handleFiltrosPersonalizadosChange(
                                filtro,
                                e.target.checked,
                              )
                            }
            ...
            <article className="relative">
              ...
              >
                ...
                <ChevronDown
                  className={`w-3 h-3 text-teal-600 transition-transform duration-200 ml-1 flex-shrink-0 ${openDropdowns.estado ? "rotate-180" : ""}`}
                />
              </button>
            ...
            <article className="relative">
              ...
              >
                ...
                <ChevronDown
                  className={`w-3 h-3 text-purple-600 transition-transform duration-200 ml-1 flex-shrink-0 ${openDropdowns.fuente_financiamiento ? "rotate-180" : ""}`}
                />
              </button>
                ...
                <section className="absolute top-full left-0 right-0 mt-1 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 rounded-lg shadow-xl z-[9999] max-h-80 overflow-y-auto">
                  <section className="p-3">
                    <input
                      ...
                      onChange={(e) =>
                        setFuenteFinanciamientoSearch(e.target.value)
                      }
                      ...
                      {filteredFuentesFinanciamiento.map((fuente: string) => (
                        ...
                        >
                          <input
                            type="checkbox"
                            checked={
                              safeFilters.fuentesFinanciamiento?.includes(
                                fuente,
                              ) || false
                            }
                            onChange={(e) =>
            ...
            <article className="relative">
              ...
              >
                ...
                <ChevronDown
                  className={`w-3 h-3 text-blue-600 transition-transform duration-200 ml-1 flex-shrink-0 ${openDropdowns.comunas_barrios ? "rotate-180" : ""}`}
                />
              </button>
                ...
                >
                  <section className="p-3">
                    ...
                    <section className="mb-4">
                      <h4 className="text-sm font-semibold text-blue-700 dark:text-blue-300 mb-2 border-b border-blue-200 dark:border-blue-700 pb-1">
                        Comunas
                      </h4>
                      <section className="mb-2">
                        <input
                        ...
                        {filteredComunas.map((comuna) => (
                          ...
                          >
                            <input
                              ...
                              className="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500"
                              checked={
                                safeFilters.comunas?.includes(comuna) || false
                              }
                              onChange={(e) => {
                                e.stopPropagation();
                          ...
                          filteredBarrios.map((barrio) => (
                            ...
                            >
                              <input
                                ...
                                checked={
                                  safeFilters.barrios?.includes(barrio) || false
                                }
                                onChange={(e) => {
                                  e.stopPropagation();
            ...
            <article className="relative">
              ...
              >
                ...
                <ChevronDown
                  className={`w-3 h-3 text-orange-600 transition-transform duration-200 ml-1 flex-shrink-0 ${openDropdowns.corregimientos_veredas ? "rotate-180" : ""}`}
                />
              </button>
              ...
              {openDropdowns.corregimientos_veredas && (
                ...
                >
                  <section className="p-3">
                    <section className="mb-4">
                      <h4 className="text-sm font-semibold text-orange-700 dark:text-orange-300 mb-2 border-b border-orange-200 dark:border-orange-700 pb-1">
                        Corregimientos
                      </h4>
                      ...
                      <section className="mb-2">
                        <input
                          ...
                          onChange={(e) =>
                            setCorregimientosSearch(e.target.value)
                          }
                        ...
                        {filteredCorregimientos.map((corregimiento) => (
                          ...
                          >
                            <input
                              ...
                              checked={
                                safeFilters.corregimientos?.includes(
                                  corregimiento,
                                ) || false
                              }
                              onChange={(e) => {
                    ...
                    <section>
                      <p className="text-sm font-semibold text-purple-700 dark:text-purple-300 mb-2 border-b border-purple-200 dark:border-purple-700 pb-1 flex items-center justify-between">
                        <span>Veredas</span>
                        {(!safeFilters.corregimientos ||
                          safeFilters.corregimientos.length === 0) && (
                          <span className="text-xs text-gray-500 italic">
                            Selecciona primero un corregimiento
                          </span>
                        )}
                        {safeFilters.corregimientos &&
                          safeFilters.corregimientos.length > 0 &&
                          displayedVeredas.length === 0 && (
                      ...
                      <section
                        className={`space-y-1 max-h-32 overflow-y-auto ${!safeFilters.corregimientos || safeFilters.corregimientos.length === 0 ? "opacity-50" : ""}`}
                      >
                          ...
                          filteredVeredas.map((vereda) => (
                            ...
                            >
                              <input
                                ...
                                className="w-4 h-4 text-purple-600 border-gray-300 rounded focus:ring-purple-500"
                                checked={
                                  safeFilters.veredas?.includes(vereda) || false
                                }
                                onChange={(e) => {
                                  e.stopPropagation();
                                  handleCheckboxChange(
              ...
              <section className="flex flex-wrap gap-2">
                {safeFilters.search && (
                  <span className="inline-flex items-center gap-1 px-3 py-1 bg-gray-100 dark:bg-gray-700 text-gray-800 dark:text-gray-200 text-sm rounded-full border border-gray-200 dark:border-gray-600">
                    ...
                    BÃºsqueda: {safeFilters.search}
                    <button
                      onClick={() => removeFilter("search")}
                      className="hover:bg-gray-200 dark:hover:bg-gray-600 rounded-full p-1 transition-colors duration-200"
                    >
                      <X className="w-3 h-3" />
                    </button>
                  </span>
                ...
                {safeFilters.centroGestor &&
                  Array.isArray(safeFilters.centroGestor) &&
                  safeFilters.centroGestor.length > 0 &&
                  safeFilters.centroGestor.map((centro) => (
                    <span
                      key={centro}
                      className="inline-flex items-center gap-1 px-3 py-1 bg-teal-100 dark:bg-teal-900 text-teal-800 dark:text-teal-200 text-sm rounded-full border border-teal-200 dark:border-teal-700"
                    >
                      <MapPin className="w-3 h-3" />
                      {centro}
                      <button
                        onClick={() => removeFilter("centroGestor", centro)}
                        className="hover:bg-teal-200 dark:hover:bg-teal-800 rounded-full p-1 transition-colors duration-200"
                      >
                        <X className="w-3 h-3" />
                      </button>
                    </span>
                ...
                {safeFilters.comunas?.map((comuna) => (
                  <span
                    key={comuna}
                    className="inline-flex items-center gap-1 px-3 py-1 bg-blue-100 dark:bg-blue-900 text-blue-800 dark:text-blue-200 text-sm rounded-full border border-blue-200 dark:border-blue-700"
                  >
                    <MapPin className="w-3 h-3" />
                    {comuna}
                    <button
                      onClick={() => removeFilter("comunas", comuna)}
                      className="hover:bg-blue-200 dark:hover:bg-blue-800 rounded-full p-1 transition-colors duration-200"
                    >
                      <X className="w-3 h-3" />
                    </button>
                ...
                {safeFilters.barrios?.map((barrio) => (
                  <span
                    key={barrio}
                    className="inline-flex items-center gap-1 px-3 py-1 bg-green-100 dark:bg-green-900 text-green-800 dark:text-green-200 text-sm rounded-full border border-green-200 dark:border-green-700"
                  >
                    <MapPin className="w-3 h-3" />
                    {barrio}
                    <button
                      onClick={() => removeFilter("barrios", barrio)}
                ...
                {safeFilters.corregimientos?.map((corregimiento) => (
                  <span
                    key={corregimiento}
                    className="inline-flex items-center gap-1 px-3 py-1 bg-orange-100 dark:bg-orange-900 text-orange-800 dark:text-orange-200 text-sm rounded-full border border-orange-200 dark:border-orange-700"
                  >
                    <MapPin className="w-3 h-3" />
                    {corregimiento}
                    <button
                      onClick={() =>
                        removeFilter("corregimientos", corregimiento)
                      }
                ...
                {safeFilters.veredas?.map((vereda) => (
                  <span
                    key={vereda}
                    className="inline-flex items-center gap-1 px-3 py-1 bg-purple-100 dark:bg-purple-900 text-purple-800 dark:text-purple-200 text-sm rounded-full border border-purple-200 dark:border-purple-700"
                  >
                    <MapPin className="w-3 h-3" />
                    {vereda}
                    <button
                      onClick={() => removeFilter("veredas", vereda)}
                ...
                {safeFilters.fuentesFinanciamiento?.map((fuente) => (
                  ...
                  >
                    ...
                    <button
                      onClick={() =>
                        removeFilter("fuentesFinanciamiento", fuente)
                      }
                      className="hover:bg-purple-200 dark:hover:bg-purple-800 rounded-full p-1 transition-colors duration-200"
                    >
                      <X className="w-3 h-3" />
                ...
                {safeFilters.filtrosPersonalizados?.map((filtro) => (
                  <span
                    key={filtro}
                    className="inline-flex items-center gap-1 px-3 py-1 bg-pink-100 dark:bg-pink-900 text-pink-800 dark:text-pink-200 text-sm rounded-full border border-pink-200 dark:border-pink-700"
                  >
                    <Filter className="w-3 h-3" />
                    {filtro}
                    <button
                      onClick={() =>
                        removeFilter("filtrosPersonalizados", filtro)
                      }
                      className="hover:bg-pink-200 dark:hover:bg-pink-800 rounded-full p-1 transition-colors duration-200"
                    >
                      <X className="w-3 h-3" />
                    </button>
                  </span>
                ...
                {safeFilters.periodos?.map((periodo) => (
                  <span
                    key={periodo}
                    className="inline-flex items-center gap-1 px-3 py-1 bg-indigo-100 dark:bg-indigo-900 text-indigo-800 dark:text-indigo-200 text-sm rounded-full border border-indigo-200 dark:border-indigo-700"
                  >
                    <Calendar className="w-3 h-3" />
                    {periodo}
                    <button
                      onClick={() => removeFilter("periodos", periodo)}
...
// Exportar tambiÃ©n la interfaz para que otros componentes la puedan usar
