...
const ProjectModal: React.FC<ProjectModalProps> = ({
  ...
  const [pdfStage, setPdfStage] = useState<
    "preparing" | "capturing" | "generating" | "downloading" | "completed"
  >("preparing");
  ...
  const getActivityStatus = (actividad: any) => {
    ...
    const startDate = actividad.fecha_inicio_actividad
      ? new Date(actividad.fecha_inicio_actividad)
      : null;
    const endDate = actividad.fecha_fin_actividad
      ? new Date(actividad.fecha_fin_actividad)
      : null;
  ...
  const getProgresoFisico = (bpin: number) => {
    ...
    return seguimiento?.avance_proyecto_pa
      ? seguimiento.avance_proyecto_pa * 100
      : 0;
  };
  ...
  const getProgresoFinanciero = (bpin: number) => {
    ...
    if (
      ejecucion?.ejecucion &&
      ejecucion?.ppto_modificado &&
      ejecucion.ppto_modificado > 0
    ) {
      return (ejecucion.ejecucion / ejecucion.ppto_modificado) * 100;
    }
    return (
      project.progressFinanciero ||
      (project.executed / project.budget) * 100 ||
      0
    );
  };
  ...
  const getUnidadesProyecto = (bpin: number) => {
    const equipamientosDelProyecto =
      equipamientos?.filter((eq) => eq.bpin === bpin) || [];
    const infraestructuraDelProyecto =
      infraestructuraVial?.filter((inf) => inf.bpin === bpin) || [];
  ...
  const getBPFromBPIN = (bpin: string) => {
    ...
    const proyectoData = dataContext.proyectos?.find(
      (p) => p.bpin === Number(bpin),
    );
          ...
          >
              ...
              <div className="flex justify-between items-start">
                <div className="flex-1 pr-4">
                  <h2
                    ...
                    className="text-3xl font-bold mb-2 text-white break-words leading-tight"
                  >
                    {project.name}
                  </h2>
                  ...
                  <div className="text-blue-200 text-base">
                    ...
                    <span className="text-blue-100 ml-1 break-words">
                      {project.responsible}
                    </span>
                  </div>
                </div>
            ...
            <div className="overflow-y-auto max-h-[calc(95vh-200px)] bg-white dark:bg-gray-900 flex-1">
              <div className="p-3 space-y-3">
                  ...
                  <div className="space-y-3 bg-blue-50/30 dark:bg-blue-900/10 border border-blue-200 dark:border-blue-700 rounded-lg p-4">
                    ...
                    <div className="bg-white dark:bg-gray-700 border border-blue-200 dark:border-blue-600 rounded-lg p-3">
                      <div className="flex justify-between items-center mb-2">
                        ...
                        <span
                          className={`text-base font-semibold ${getProgressTextColor("physical", getProgresoFisico(Number(project.bpin)))} dark:${getProgressTextColor("physical", getProgresoFisico(Number(project.bpin)))}`}
                        >
                          {formatPercentage(
                            getProgresoFisico(Number(project.bpin)),
                          )}
                        </span>
                      </div>
                      <div className="w-full bg-gray-200 dark:bg-gray-600 rounded-full h-2">
                        <div
                          ...
                          style={{
                            width: `${Math.min(getProgresoFisico(Number(project.bpin)), 100)}%`,
                          }}
                        ></div>
                      </div>
                    </div>
                    ...
                    <div className="bg-white dark:bg-gray-700 border border-blue-200 dark:border-blue-600 rounded-lg p-3">
                      <div className="flex justify-between items-center mb-2">
                        ...
                        <span
                          className={`text-base font-semibold ${getProgressTextColor("financial", getProgresoFinanciero(Number(project.bpin)))} dark:${getProgressTextColor("financial", getProgresoFinanciero(Number(project.bpin)))}`}
                        >
                          {formatPercentage(
                            getProgresoFinanciero(Number(project.bpin)),
                          )}
                        </span>
                      </div>
                      <div className="w-full bg-gray-200 dark:bg-gray-600 rounded-full h-2">
                        <div
                          ...
                          style={{
                            width: `${Math.min(getProgresoFinanciero(Number(project.bpin)), 100)}%`,
                          }}
                        ></div>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                      ...
                      <div className="flex flex-col items-center justify-center text-center p-4 bg-white dark:bg-gray-700 rounded-lg border">
                        ...
                        <div className="text-lg font-bold text-orange-600 dark:text-orange-400">
                          {formatCurrency(
                            Math.max(0, project.budget - project.executed),
                          )}
                        </div>
                      </div>
                    </div>
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                            {proyectoData.nombre_centro_gestor && (
                              <div className="bg-white dark:bg-gray-700 p-2 rounded border border-blue-200 dark:border-blue-600">
                                <span className="text-blue-700 dark:text-blue-300 font-medium text-xs block">
                                  Centro Gestor
                                </span>
                          ...
                          <table className="min-w-full divide-y divide-orange-200 dark:divide-orange-600">
                            ...
                            <tbody className="bg-white dark:bg-gray-800 divide-y divide-orange-200 dark:divide-orange-600">
                              {productos.map((producto, index) => {
                                ...
                                const statusKey =
                                  statusInfo.status as keyof typeof statusColors;
                                const progresoProducto =
                                  (producto.avance_real_producto || 0) * 100;
                                const ejecucionProducto =
                                  producto.ejecucion_ppto_producto &&
                                  producto.ppto_inicial_producto
                                    ? (producto.ejecucion_ppto_producto /
                                        producto.ppto_inicial_producto) *
                                      100
                                    : 0;
                                    ...
                                    <td className="px-4 py-5 align-top border-l border-orange-200 dark:border-orange-600 w-[30%]">
                                      <div className="space-y-3">
                                        ...
                                        <div className="flex flex-col space-y-1">
                                          ...
                                          <span
                                            className={`text-xs font-medium ${statusColors[statusKey]} break-words`}
                                          >
                                            {statusInfo.status}
                                          </span>
                                        </div>
                                        ...
                                        <div className="space-y-2 text-xs">
                                          <div className="flex justify-between">
                                            ...
                                            <span className="font-medium text-gray-900 dark:text-white">
                                              {producto.cantidad_programada_producto
                                                ? producto.cantidad_programada_producto.toLocaleString()
                                                : "—"}
                                            </span>
                                          </div>
                                          <div className="flex justify-between">
                                        ...
                                        <div className="pt-2 border-t border-gray-100 dark:border-gray-600">
                                          ...
                                          <div className="w-full bg-gray-200 dark:bg-gray-600 rounded-full h-1.5">
                                            <div
                                              ...
                                              style={{
                                                width: `${Math.min(progresoProducto, 100)}%`,
                                              }}
                                            ></div>
                                          </div>
                                        </div>
                                        ...
                                        <div>
                                          ...
                                          <div className="w-full bg-gray-200 dark:bg-gray-600 rounded-full h-1.5">
                                            <div
                                              ...
                                              style={{
                                                width: `${Math.min(ejecucionProducto, 100)}%`,
                                              }}
                                            ></div>
                                          </div>
                                        </div>
                ...
                {/* Actividades */}
                {(() => {
                  const actividades = getActividadesByBpin(
                    Number(project.bpin),
                  );
                  return (
                            ...
                            <tbody className="bg-white dark:bg-gray-900 divide-y divide-red-200 dark:divide-red-700">
                              {actividades.map((actividad, index) => {
                                ...
                                const progresoActividad =
                                  (actividad.avance_actividad_acumulado || 0) *
                                  100;
                                const ejecucionActividad =
                                  actividad.ejecucion_actividad &&
                                  actividad.ppto_inicial_actividad
                                    ? (actividad.ejecucion_actividad /
                                        actividad.ppto_inicial_actividad) *
                                      100
                                    : 0;
                                        ...
                                        <div className="pt-2 border-t border-gray-100 dark:border-gray-600">
                                          ...
                                          <div className="w-full bg-gray-200 dark:bg-gray-600 rounded-full h-1.5">
                                            <div
                                              ...
                                              style={{
                                                width: `${Math.min(progresoActividad, 100)}%`,
                                              }}
                                            ></div>
                                          </div>
                                        </div>
                                        ...
                                        <div>
                                          ...
                                          <div className="w-full bg-gray-200 dark:bg-gray-600 rounded-full h-1.5">
                                            <div
                                              ...
                                              style={{
                                                width: `${Math.min(ejecucionActividad, 100)}%`,
                                              }}
                                            ></div>
                                          </div>
                                        </div>
                        ...
                        <div className="space-y-3">
                          ...
                          <div className="max-h-32 overflow-y-auto space-y-1">
                            {unidades.slice(0, 5).map((unidad, index) => (
                              <div
                                key={
                                  unidad.upid || unidad.id || `unidad-${index}`
                                }
                                className="bg-white dark:bg-gray-700 p-2 rounded border text-xs"
                              >
                                <div className="font-medium text-gray-900 dark:text-white">
                                  {unidad.nombre ||
                                    unidad.nombre_equipamiento ||
                                    "Unidad sin nombre"}
                                </div>
            ...
            <div className="border-t border-gray-200 dark:border-gray-700 p-3 bg-gray-50 dark:bg-gray-800/70">
              <div className="flex justify-between items-center">
                <div className="text-xs text-gray-500 dark:text-gray-400 flex-1">
                  Ficha generada el {new Date().toLocaleDateString("es-CO")} a
                  las{" "}
