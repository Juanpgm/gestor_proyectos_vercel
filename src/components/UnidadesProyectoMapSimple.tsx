...
// Tipos de coloraciÃ³n disponibles
type ColoringType =
  | "estado"
  | "tipo_intervencion"
  | "tipo_equipamiento"
  ...
  useEffect(() => {
    if (focusedItem && geometryData?.features) {
      ...
      if (targetFeature) {
        ...
        // Calcular centro de la geometrÃ­a segÃºn su tipo
        let latLng: [number, number] | null = null;

        try {
          if (geomType === "Point") {
            const coords = targetFeature.geometry.coordinates as [
              number,
              number,
            ];
            ...
            // Calcular el punto medio de la lÃ­nea
            ...
            // Calcular el centroide del polÃ­gono (primer anillo)
            const ring = coords[0];
            ...
            // Usar el punto medio de la primera lÃ­nea
            ...
            // Usar el centroide del primer polÃ­gono
            if (coords.length > 0 && coords[0].length > 0) {
            ...
            // Para GeometryCollection, usar la primera geometrÃ­a
            ...
            if (geometries && geometries.length > 0) {
              ...
              if (firstGeom.type === "Point") {
                ...
                firstGeom.type === "LineString" &&
                firstGeom.coordinates.length > 0
              ) {
                const midIndex = Math.floor(firstGeom.coordinates.length / 2);
                latLng = [
                  firstGeom.coordinates[midIndex][1],
                  firstGeom.coordinates[midIndex][0],
                ];
              }
            }
          }
        } catch (error) {
          console.warn("Error al calcular centro de geometrÃ­a:", error);
        }

        // Enfocar en el elemento con zoom y animaciÃ³n
        if (latLng) {
...
// Componente para aÃ±adir etiquetas de texto a las capas base
const BaseLayerLabels: React.FC<{
  ...
  // Calcular el centroide de cada polÃ­gono para posicionar la etiqueta
  const labels = data.features
    .map((feature: any, index: number) => {
      const properties = feature.properties;
      const name =
        properties?.NOMBRE || properties?.nombre || properties?.name || "";
                    ...
                    <div className="space-y-1.5">
                      ...
                      <label className="flex items-center space-x-2 cursor-pointer group">
                        <input
                          ...
                          onChange={() =>
                            onColorModeChange("multitone-vibrant")
                          }
                          className="w-3.5 h-3.5 text-blue-600 border-gray-300 focus:ring-blue-500 dark:border-gray-600 dark:focus:ring-blue-600"
                        />
                        <span className="text-xs text-gray-700 dark:text-gray-300 group-hover:text-gray-900 dark:group-hover:text-white">
...
// Componente de control de coloraciÃ³n
const ColoringControl: React.FC<{
  ...
  // Generar opciones dinÃ¡micamente basadas en valores Ãºnicos de los datos
  const coloringOptions = useMemo(() => {
    // FunciÃ³n helper para extraer valores Ãºnicos
  ...
  return (
    <div className="absolute top-4 right-4 z-[1000] space-y-2">
      ...
      {/* Leyenda */}
      <div className="bg-white dark:bg-gray-800 rounded-md shadow-none border border-gray-200 dark:border-gray-700 p-3 max-w-sm">
...
const UnidadesProyectoMapSimple: React.FC<UnidadesProyectoMapSimpleProps> = ({
  ...
  useEffect(() => {
    ...
    console.log("UnidadesProyectoMapSimple: Received data:", {
    ...
    if (geometryData && geometryData.features) {
      if (geometryData.features.length === 0) {
        console.warn("Map has 0 features to display!");
      } else if (validGeometries === 0) {
        console.warn(
          "All features have invalid coordinates [0,0] - cannot display on map!",
        );
      }
    }
  ...
  const [baseLayerColorMode, setBaseLayerColorMode] =
    useState<BaseLayerColorMode>("monotone");
  const [baseLayerMonotoneColor, setBaseLayerMonotoneColor] =
  ...
  // FunciÃ³n para obtener el estilo de las capas base segÃºn la configuraciÃ³n
  const getBaseLayerStyle = (feature: any) => {
    if (baseLayerColorMode === "monotone") {
      ...
      // Seleccionar la paleta segÃºn el modo
      ...
      // Generar un Ã­ndice basado en el ID y nombre del feature para tener colores consistentes
      // Usar mÃºltiples propiedades para maximizar la variaciÃ³n de colores
      ...
      // Crear una cadena Ãºnica combinando todas las propiedades disponibles
      const uniqueString = `${featureId}-${featureName}-${featureCode}`;

      // FunciÃ³n de hash mejorada (DJB2)
      let hash = 5381;
  ...
  // Helper para normalizar UPIDs para comparaciÃ³n case-insensitive
  const normalizeUpid = (upid: string | null | undefined): string => {
    if (!upid) return "";
    return String(upid).trim().toLowerCase();
  };

  // Filtrar datos segÃºn el item enfocado
  const displayData = useMemo(() => {
    if (showOnlyFocused && focusedItem) {
      const normalizedFocused = normalizeUpid(focusedItem);
      return filteredData.filter(
        (item) => normalizeUpid(item.upid) === normalizedFocused,
      );
    }
    return filteredData;
  }, [filteredData, showOnlyFocused, focusedItem]);

  // Consolidar datos por UPID para coloraciÃ³n (tipo, avance, centro, inversiÃ³n)
  const consolidatedColorData = useMemo(() => {
    ...
    return Array.from(grouped.values()).map((group) => {
      ...
      const fuentes = new Set(
        group.map((i) => i.fuente_financiacion).filter(Boolean),
      );
      const avances = group
        .map((i) => i.avance_obra)
        .filter(
  ...
  // Generar esquema de colores y leyenda basado en el tipo de coloraciÃ³n
    ...
    switch (coloringType) {
      case "avance_obra": {
        ...
        data.forEach((item) => {
          ...
          const range =
            ranges.find((r) => avance >= r.min && avance <= r.max) || ranges[0];
      ...
      case "presupuesto_base": {
        ...
        const ranges = [
          {
            min: 0,
            max: quartiles[0],
            color: COLOR_SCHEMES.presupuesto[0],
            label: `$0 - $${(quartiles[0] / 1000000).toFixed(1)}M`,
          },
          {
            min: quartiles[0],
            max: quartiles[1],
            color: COLOR_SCHEMES.presupuesto[2],
            label: `$${(quartiles[0] / 1000000).toFixed(1)}M - $${(quartiles[1] / 1000000).toFixed(1)}M`,
          },
          {
            min: quartiles[1],
            max: quartiles[2],
            color: COLOR_SCHEMES.presupuesto[4],
            label: `$${(quartiles[1] / 1000000).toFixed(1)}M - $${(quartiles[2] / 1000000).toFixed(1)}M`,
          },
          {
            min: quartiles[2],
            max: Infinity,
            color: COLOR_SCHEMES.presupuesto[5],
            label: `> $${(quartiles[2] / 1000000).toFixed(1)}M`,
          },
        ];
        ...
        data.forEach((item) => {
          ...
          const range =
            ranges.find((r) => amount >= r.min && amount < r.max) || ranges[0];
          colorMap.set(normalizeUpid(item.upid), range.color);
        });
        ...
        const legend = ranges.map((range) => ({
          ...
          count: data.filter((item) => {
            ...
            return (
              amount >= range.min &&
              (range.max === Infinity ? true : amount < range.max)
            );
          }).length,
        }));
      ...
      default: {
        ...
        const uniqueValues = Array.from(
          new Set(
            data
              .map((item) =>
                normalizeCategoricalValue(String(item[field] || "")),
              )
              .filter(Boolean),
          ),
        );
        ...
        uniqueValues.forEach((value, index) => {
          const color = isEstadoColoring
            ...
            : COLOR_SCHEMES.categorical[
                index % COLOR_SCHEMES.categorical.length
              ];

          data
            .filter(
              (item) =>
                normalizeCategoricalValue(String(item[field] || "")) === value,
            )
  ...
  const getFeatureStyle = (feature: any) => {
    ...
    // Ajustar peso segÃºn el tipo de geometrÃ­a
    if (geomType === "LineString" || geomType === "MultiLineString") {
      // LÃ­neas mÃ¡s gruesas para mejor visibilidad
      baseStyle.weight = isFocused ? 6 : 4;
      baseStyle.opacity = isDimmed ? 0.3 : isFocused ? 1 : 0.9;
    } else if (geomType === "Polygon" || geomType === "MultiPolygon") {
      // PolÃ­gonos con borde visible
  ...
  // FunciÃ³n especÃ­fica para marcadores circulares
  ...
  // FunciÃ³n para validar si una geometrÃ­a tiene coordenadas vÃ¡lidas
  const hasValidCoordinates = (geometry: any): boolean => {
    ...
    try {
      switch (geomType) {
        ...
        default:
          // Para tipos desconocidos, asumir vÃ¡lido si tiene coordenadas
          return coords != null;
      }
    } catch (error) {
  ...
  // Verificar si hay features con geometrÃ­as vÃ¡lidas
  const validFeatures =
    geometryData?.features?.filter(
      (f) =>
        f.properties.has_valid_geometry !== false &&
  ...
  return (
    ...
    >
      {/* Alerta cuando no hay geometrÃ­as vÃ¡lidas */}
            ...
            <div className="flex items-start">
              ...
              <div className="ml-3">
                ...
                <p className="text-xs text-yellow-600 dark:text-yellow-300 mt-1">
                  ...
                  pero sin ubicación en el mapa. Verifica la tabla para ver los
                  datos.
                </p>
              </div>
            </div>
      ...
      <div className="absolute top-4 left-4 z-[1000]">
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-1 border border-gray-200 dark:border-gray-700">
          <button
            ...
            title={
              mapType === "streets" ? "Vista satelital" : "Vista de calles"
            }
          >
            {mapType === "streets" ? (
      ...
      {/* Control de coloraciÃ³n y leyenda */}
      <ColoringControl
        coloringType={coloringType}
      ...
      {focusedItem && (
        <div className="absolute top-4 right-4 z-[1000]">
          <div className="bg-white dark:bg-gray-800 rounded-md shadow-none border border-gray-200 dark:border-gray-700 p-3">
      ...
      <div className="absolute bottom-4 left-4 z-[1000] bg-white dark:bg-gray-800 rounded-lg shadow-lg px-3 py-2 border border-gray-200 dark:border-gray-700">
        <div className="flex items-center space-x-2">
          ...
          <span className="text-sm font-medium text-gray-900 dark:text-white">
            ...
            {geometryData?.features?.length || 0} con ubicación)
          </span>
        </div>
      </div>
                    ...
                    ">
                      ...
                      ${
                        ...
                        <div style="
                          ...
                          gap: 6px;
                        ">
                          <span style="opacity: 0.7;">Equipamiento:</span>
                          <span style="font-weight: 500; color: ${isDark ? "#d1d5db" : "#374151"};">
                            ${attributeItem.tipo_equipamiento}
                          </span>
                        </div>
                      ...
                      <div style="
                        ...
                        gap: 6px;
                      ">
                        <span style="opacity: 0.7;">Ubicacion:</span>
                        <div>
                      ...
                      ${
                        ...
                        <div style="
                          ...
                          gap: 6px;
                        ">
                          <span style="opacity: 0.7;">Presupuesto:</span>
