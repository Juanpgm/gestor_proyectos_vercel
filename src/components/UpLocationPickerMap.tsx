...
import { getTileAttribution, getTileUrl } from "@/lib/map-theme";
...
const DEFAULT_CENTER: LatLng = [3.4516, -76.532]; // Cali, CO
...
const MapClickHandler: React.FC<{ onSelect: (position: LatLng) => void }> = ({
  onSelect,
}) => {
  useMapEvents({
    click(event) {
...
const UpLocationPickerMap: React.FC<UpLocationPickerMapProps> = ({
  ...
  return (
    <div className="ct-map-shell h-72 w-full overflow-hidden rounded-md border border-gray-300 dark:border-gray-700">
      <MapContainer
        center={center}
        zoom={13}
