...
const SolicitudesPendientesTab: React.FC = () => {
  ...
  const [solicitudesIntervencion, setSolicitudesIntervencion] = useState<
    SolicitudCambio[]
  >([]);
  ...
  const removeSolicitudFromList = (
    list: SolicitudCambio[],
  ...
  const handleAprobar = async (solicitud: SolicitudCambio) => {
    ...
    try {
      ...
      if (solicitud.tipo === "unidad_proyecto") {
        ...
        if (!intervencionId)
          throw new Error("Falta el intervencion_id en la solicitud");
        if (!solicitudId) throw new Error("Falta el ID de la solicitud");
        await aplicarCambiosFirestore(
          "intervencion",
      ...
      if (solicitud.tipo === "unidad_proyecto") {
        ...
        setSolicitudesIntervencion((prev) =>
          removeSolicitudFromList(prev, solicitud),
        );
      }
    } catch (err) {
  ...
  const handleRechazar = async (solicitud: SolicitudCambio) => {
    if (
      !confirm(
        "¿Deseas rechazar esta solicitud de cambio? Se marcará como rechazada en el sistema.",
      )
    )
      return;
    const solicitudId = getSolicitudId(solicitud);
          ...
          {allSolicitudes.map((sol, index) => {
            ...
            return (
              <div
                key={
                  solId ||
                  `${sol.tipo}-${sol.upid || sol.intervencion_id || index}`
                }
