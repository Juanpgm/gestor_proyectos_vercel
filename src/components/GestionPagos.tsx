...
const GestionPagos: React.FC<GestionPagosProps> = ({ onNavigateHome }) => {
  ...
  const stats = useMemo(() => {
    ...
    const totalValorRPCs = rpcs.reduce(
      (sum, rpc) => sum + (rpc.valor_rpc || 0),
      0,
    );
    const totalPagos = pagos.length;
    const totalValorPagos = pagos.reduce(
      (sum, pago) => sum + (pago.valor_pago || 0),
      0,
    );
  ...
  const toggleColumnFilter = (columnKey: string, value: string) => {
    setColumnFilters((prev) => {
      ...
      return newFilters.length > 0
        ...
        : Object.fromEntries(
            Object.entries(prev).filter(([k]) => k !== columnKey),
          );
    });
    setCurrentPage(1);
  };
  ...
  const columns: Column[] = [
    ...
    { key: "descripcion_rpc", label: "DescripciÃ³n", isSortable: true },
    { key: "nombre_centro_gestor", label: "Centro Gestor", isSortable: true },
    { key: "valor_rpc", label: "Valor RPC", isSortable: true },
    { key: "estado_liberacion", label: "Estado LiberaciÃ³n", isSortable: true },
    {
      key: "fecha_contabilizacion",
      label: "Fecha ContabilizaciÃ³n",
      isSortable: true,
    },
    { key: "fecha_impresion", label: "Fecha ImpresiÃ³n", isSortable: true },
      ...
      >
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div className="flex items-center space-x-4">
            <div className="bg-emerald-50 dark:bg-emerald-900/20 p-2 rounded-md border border-emerald-200 dark:border-emerald-900/30">
                      ...
                      <div className="p-3">
                        <div className="flex items-center justify-between mb-3 pb-2 border-b border-gray-200 dark:border-gray-600">
                          ...
                          <div className="flex space-x-1">
                            ...
                            <button
                              onClick={() =>
                                setVisibleColumns(new Set(["numero_rpc"]))
                              }
                              ...
                              {showFilters[column.key] && (
                                ...
                                >
                                  <div className="p-2 max-h-64 overflow-y-auto">
                                    ...
                                    {getUniqueValuesForColumn(column.key).map(
                                      (value) => (
                                        <label
                                          key={value}
                        ...
                        {visibleColumnsList.map((column) => (
                          ...
                          >
                            {column.key === "acciones" ? (
                              ...
                              formatCurrency(
                                item[column.key as keyof typeof item] as number,
                              )
                              ...
                              <span
                                className={`px-2 py-1 rounded-full text-xs font-medium ${
                                  item.estado_liberacion === "Liberado"
                                    ...
                                    : item.estado_liberacion ===
                                        "Parcialmente Liberado"
                                      ? "bg-yellow-100 dark:bg-yellow-900/30 text-yellow-700 dark:text-yellow-300"
                                      : "bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-300"
                                }`}
                              >
                                {item.estado_liberacion !== null &&
                                item.estado_liberacion !== undefined
                                  ? String(item.estado_liberacion)
                                  : ""}
                              </span>
