...
const GestionRegistrosEmprestitoTab = dynamic(
  () => import("./GestionRegistrosEmprestitoTab"),
  { ssr: false },
);
const AvancesEmprestitoTab = dynamic(() => import("./AvancesEmprestitoTab"), {
  ssr: false,
});
const SolicitudesPendientesEmprestitoTab = dynamic(
  () => import("./SolicitudesPendientesEmprestitoTab"),
  { ssr: false },
);
const HistorialSolicitudesEmprestitoTab = dynamic(
  () => import("./HistorialSolicitudesEmprestitoTab"),
  { ssr: false },
);
const AnalisisEmprestitoTab = dynamic(() => import("./AnalisisEmprestitoTab"), {
  ssr: false,
});
...
const GestionEmprestito: React.FC<GestionEmprestitoProps> = ({
  onNavigateHome,
}) => {
  ...
  const normalizeDimensionLabel = (value: any) =>
    String(value || "").trim() || "no_clasificado";
  ...
  useEffect(() => {
    ...
    if (g.centros_gestores.length > 0)
      setAvailableCentrosGestores(g.centros_gestores);
      ...
      try {
        ...
        if (!canViewAll && userCentroGestor) {
          ...
          records = records.filter((item: any) => {
            ...
            return (
              c === normUser || c.includes(normUser) || normUser.includes(c)
            );
          });
        }
        ...
        const merged: EmprestitoFilterOptions = {
          centros_gestores: normalizeOptions([
            ...g.centros_gestores,
            ...records.map(
              (r: any) => r?.nombre_centro_gestor || r?.centro_gestor,
            ),
          ]),
  ...
  const tabs = useMemo(() => {
    const base: Array<{
      ...
      {
        id: "resumen",
        label: EMPRESTITO_TAB_LABELS["resumen"],
        icon: CheckCircle2,
        description: "Resumen de calidad de datos del empréstito",
      },
      {
        id: "historial-cambios",
        label: EMPRESTITO_TAB_LABELS["historial-cambios"],
        icon: History,
        description: "Historial de cambios en control de calidad",
      },
      {
        id: "por-centro-gestor",
        label: EMPRESTITO_TAB_LABELS["por-centro-gestor"],
        icon: Building2,
        description: "Calidad y registros agrupados por centro gestor",
      },
      {
        id: "estadisticas",
        label: EMPRESTITO_TAB_LABELS["estadisticas"],
        icon: TrendingUp,
        description: "Estadísticas de calidad",
      },
    ];
    ...
    if (canViewAnalisis) {
      base.push({
        id: "analisis",
        label: EMPRESTITO_TAB_LABELS["analisis"],
        icon: BarChart3,
        description: "Análisis por banco, CG y BP",
      });
    }

    base.push(
      {
        id: "gestionar-registros",
        label: EMPRESTITO_TAB_LABELS["gestionar-registros"],
        icon: FilePenLine,
        description: "Crear, eliminar y solicitar cambios",
      },
      {
        id: "avances",
        label: EMPRESTITO_TAB_LABELS["avances"],
        icon: FileText,
        description: "Avances por centro gestor",
      },
    );
      ...
      base.push(
        {
          ...
          label: EMPRESTITO_TAB_LABELS["solicitudes-pendientes"],
          icon: ShieldCheck,
          description: "Aprobar/rechazar solicitudes",
        },
        {
          id: "historial-solicitudes",
          label: EMPRESTITO_TAB_LABELS["historial-solicitudes"],
          icon: Archive,
  ...
  const buildRecords = (source: any) => {
    ...
    return registros.map((reg: any, index: number) => {
      const diagnostico =
        pickFirst(
          reg?.diagnostico,
          reg?.diagnostico_calidad,
          reg?.quality_diagnostics,
          {},
        ) || {};
      ...
      const issues = Array.isArray(rawIssues)
        ? rawIssues.map((issue: any, i: number) => ({
            rule_id: String(
              pickFirst(issue?.rule_id, issue?.codigo_regla, `R-${i + 1}`),
            ),
            rule_name: String(
              pickFirst(
                issue?.rule_name,
            ...
            dimension: normalizeDimensionLabel(
              pickFirst(issue?.dimension, issue?.categoria),
            ),
            severity: mapSeverityCode(pickFirst(issue?.severity, issue?.nivel)),
            field_name: String(
              pickFirst(issue?.field_name, issue?.campo, "N/A"),
            ),
            ...
            details: String(
              pickFirst(issue?.details, issue?.detalle, "Sin detalles"),
            ),
          }))
        : [];
      ...
      issues.forEach((iss: any) => {
        ...
        dimensionCounts[iss.dimension] =
          (dimensionCounts[iss.dimension] || 0) + 1;
      });
      ...
      return {
        ...
        upid: String(
          pickFirst(
            reg?.referencia_contrato,
            reg?.referencia_proceso,
            reg?.id,
            `EMP-${index + 1}`,
          ),
        ),
        ...
        max_severity:
          issues.length > 0 ? mapSeverityCode(issues[0].severity) : "LOW",
        priority: mapPriorityCode(
          pickFirst(diagnostico?.priority, reg?.priority),
        ),
        issues,
        affected_fields: Array.from(
          new Set(issues.map((i: any) => i.field_name).filter(Boolean)),
        ),
  ...
  const buildSummary = (source: any) => {
    ...
    const recordsWithIssues = records.filter(
      (r: any) => toNumber(r.total_issues) > 0,
    );

    const severityDist = records.reduce(
      (acc: Record<string, number>, r: any) => {
        Object.entries(r.severity_counts || {}).forEach(([s, c]) => {
          acc[s] = (acc[s] || 0) + toNumber(c);
        });
        return acc;
      },
      {},
    ...
    const dimensionDist = records.reduce(
      (acc: Record<string, number>, r: any) => {
        Object.entries(r.dimension_counts || {}).forEach(([d, c]) => {
          acc[d] = (acc[d] || 0) + toNumber(c);
        });
        return acc;
      },
      {},
    ...
    return {
      ...
      requires_immediate_action: ["CRITICAL", "WARNING"].includes(
      ...
      recommendations: (Array.isArray(resumen.hallazgos_principales)
  ...
  const buildCentros = (source: any) => {
    ...
    if (centros.length === 0) {
      ...
      return Array.from(grouped.entries()).map(([nombre, recs], idx) => {
        ...
        const withIssues = recs.filter(
          (r) => toNumber(r.total_issues) > 0,
        ).length;
        const totalIssues = recs.reduce(
          (a, r) => a + toNumber(r.total_issues),
          0,
        );
        const score =
          total > 0 ? Number(((1 - withIssues / total) * 100).toFixed(2)) : 100;
        return {
          id: `${nombre}-${idx}`,
          ...
          quality_score: score,
          error_rate:
            total > 0 ? Number(((withIssues / total) * 100).toFixed(2)) : 0,
          status:
            score >= 95 ? "OPTIMO" : score >= 85 ? "ACEPTABLE" : "CRITICAL",
  ...
  const buildChangelog = (source: any) => {
    const root =
      pickFirst(source.historial, source.changelog, source.cambios) || {};
      ...
      data: {
        ...
        total_hallazgos: {
          ...
          count: records.reduce(
            (a: number, r: any) => a + toNumber(r.total_issues),
            0,
          ),
        },
      },
  ...
  const extractTabData = (result: any, tab: EmprestitoTabId) => {
    ...
    if (tab === "resumen") {
      ...
      const centrosList = Object.entries(groupedByCG).map(
        ([nombre, info]: [string, any]) => ({
          ...
          quality_score:
            info.total_records > 0
              ? Number(
                  (
                    (1 - (info.with_issues || 0) / info.total_records) *
                    100
                  ).toFixed(2),
                )
              : 100,
          error_rate:
            info.total_records > 0
              ? Number(
                  (
                    ((info.with_issues || 0) / info.total_records) *
                    100
                  ).toFixed(2),
                )
              : 0,
      ...
      if (duplicateIssueCount > 0 && rawSevDist.CRITICAL) {
        rawSevDist.CRITICAL = Math.max(
          0,
          rawSevDist.CRITICAL - duplicateIssueCount,
        );
      }
      const adjustedTotalIssues = Object.values(rawSevDist).reduce(
        (a, b) => a + b,
        0,
      );
      ...
      const summaryData = {
        ...
        requires_immediate_action: ["CRITICAL", "WARNING", "CRITICO"].includes(
          status,
        ),
    ...
    if (tab === "registros") {
      ...
      if (records.length === 0 && !source.rules)
        return { found: false, payload: null };
      ...
      const getIssueMeta = (reason: string, field: string) => {
        ...
        > = {
          "missing_required::monto_pagado": {
            ...
            description:
              'El campo "monto_pagado" es obligatorio pero está vacío o no existe en este registro de pago.',
            suggestion:
              'Abrir el registro del pago en Gestionar Registros → Pagos, editar y completar el campo "monto_pagado" con el valor efectivamente desembolsado.',
          },
          "duplicate_reference::referencia_contrato": {
            rule_id: "EMP-PAGO-003",
            rule_name: "Unicidad de referencia en pago",
            description:
              'El valor de "referencia_contrato" aparece en más de un registro de pago, lo que indica posible duplicación de registros.',
            suggestion:
              "Revisar los pagos que comparten la misma referencia_contrato. Si son pagos diferentes, asignar un identificador único a cada uno. Si son duplicados, eliminar el registro sobrante desde Gestionar Registros → Pagos.",
          },
          "orphan_contract_reference::referencia_contrato": {
            rule_id: "EMP-PAGO-004",
            rule_name: "Integridad referencial en pago",
            description:
              'El valor de "referencia_contrato" no corresponde a ningún contrato registrado en la colección de contratos del empréstito.',
            suggestion:
              "Verificar que la referencia_contrato sea correcta. Si el contrato existe con una referencia diferente, corregir el valor en el pago. Si el contrato aún no se ha registrado, crearlo primero en Gestionar Registros → Contratos.",
          },
        };
      ...
      const mapped = records.map((rec: any, idx: number) => {
        ...
        issues.forEach((iss: any) => {
          severityCounts[iss.severity] =
            (severityCounts[iss.severity] || 0) + 1;
        });
        return {
          ...
          nombre_centro_gestor:
            rec.nombre_centro_gestor || "Centro no especificado",
          record_type: rec.record_type || "registro",
          source_doc_id: rec.source_doc_id || "",
          max_severity:
            issues.length > 0
              ? issues.reduce((worst: string, iss: any) => {
                  ...
                  return (sevOrder[iss.severity] ?? 99) <
                    (sevOrder[worst] ?? 99)
                    ? iss.severity
                    : worst;
                }, "LOW")
      ...
      mapped.forEach((rec: any) => {
        ...
        if (!byContract.has(key)) {
          ...
          const existingRuleIds = new Set(
            existing.issues.map((i: any) => i.rule_id),
          );
          ...
          if (
            (sevOrder[rec.max_severity] ?? 99) <
            (sevOrder[existing.max_severity] ?? 99)
          ) {
          ...
          existing.affected_fields = Array.from(
            new Set(
              existing.issues.map((i: any) => i.field_name).filter(Boolean),
            ),
          );
        }
      });
    ...
    if (tab === "por-centro-gestor") {
      ...
      const mapped = centros.map((c: any, idx: number) => {
        ...
        return {
          id: `${c.nombre_centro_gestor || "centro"}-${idx}`,
          nombre_centro_gestor:
            c.nombre_centro_gestor || "Centro no especificado",
          total_records: total,
          records_with_issues: withIssues,
    ...
    if (tab === "estadisticas") {
      ...
      Object.entries(collections).forEach(([key, val]: [string, any]) => {
        dataMap[key] = {
          count: val.total_documents || 0,
          collection: val.collection || key,
        };
      });
      dataMap["total_calidad"] = {
        count: latestReport.total_records || 0,
        collection: "quality_reports",
      };
      dataMap["con_problemas"] = {
        count: latestReport.records_with_issues || 0,
        collection: "records_with_issues",
      };
      dataMap["reportes_calidad"] = {
        count: stats.total_quality_reports || 0,
        collection: "quality_reports_total",
      };
  ...
  const loadData = async () => {
    const autonomousTabs: EmprestitoTabId[] = [
      "gestionar-registros",
      "avances",
    ...
    try {
      ...
      if (!endpoint) {
        throw new Error(
          `No hay endpoint configurado para "${EMPRESTITO_TAB_LABELS[activeTab]}"`,
        );
      }
      ...
      if (!adapted.found || !adapted.payload) {
        throw new Error(
          `No se encontraron datos para "${EMPRESTITO_TAB_LABELS[activeTab]}"`,
        );
      }
      ...
      if (activeTab === "por-centro-gestor") {
        try {
          ...
          while (hasMore) {
            ...
            const recordsAdapted = extractTabData(
              recordsResult,
              "registros" as EmprestitoTabId,
            );
            if (
              recordsAdapted.found &&
              recordsAdapted.payload?.success &&
              Array.isArray(recordsAdapted.payload.data)
            ) {
          ...
          console.warn(
            "No se pudieron cargar registros para detalle por centro gestor",
          );
          setRecordsData([]);
        }
      }
        ...
        const merged: EmprestitoFilterOptions = {
          centros_gestores:
            g.centros_gestores.length > 0
              ...
              : normalizeOptions(
                  arr.map(
                    (i: any) => i.nombre_centro_gestor || i.centro_gestor,
                  ),
                ),
          estados:
            g.estados.length > 0
              ? g.estados
              : normalizeOptions(
                  arr.map((i: any) => i.estado || i.max_severity),
                ),
          bancos:
            g.bancos.length > 0
              ? g.bancos
    ...
    if (selectedEstados.length > 0) {
      filtered = filtered.filter(
        (item) =>
          ...
          selectedEstados.includes(item.max_severity),
      );
    }
        ...
        >
          ...
          {!loading && (
            <div className="flex-1 min-h-0 overflow-auto p-4">
                  ...
                  <>
                    <div className="flex justify-end mb-3">
                      <button
                        onClick={async () => {
                          try {
                            const { exportarEmprestitoXLSX } =
                              await import("@/services/emprestito-gestion.service");
              ...
              {/* Análisis */}
              {canViewAnalisis && activeTab === "analisis" && (
                <AnalisisEmprestitoTab />
              )}

              {/* Gestionar registros */}
              {activeTab === "gestionar-registros" && (
                <GestionRegistrosEmprestitoTab />
              )}
              ...
              {canViewSolicitudesTabs &&
                activeTab === "solicitudes-pendientes" && (
                  <SolicitudesPendientesEmprestitoTab
                    onRefresh={handleRefreshFromTab}
                  />
                )}
