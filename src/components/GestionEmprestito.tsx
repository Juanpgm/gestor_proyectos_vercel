        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-600 dark:text-slate-400">
                Gestión unificada de contratos, procesos, RPCs, pagos y convenios
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            >
              ...
              Filtros
              {(selectedCentrosGestores.length + selectedEstados.length + selectedBancos.length + (searchTerm ? 1 : 0)) > 0 && (
                <span className="px-2 py-0.5 rounded-full bg-blue-600 text-white text-xs font-semibold">
                  {selectedCentrosGestores.length + selectedEstados.length + selectedBancos.length + (searchTerm ? 1 : 0)}
                </span>
              )}
            </button>
            <button
          ...
          <div className="space-y-4">
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-10 pr-10 py-2 border border-slate-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100 placeholder-slate-400 focus:ring-2 focus:ring-blue-500"
              />
              {searchTerm && (
                <button
            ...
            {(selectedCentrosGestores.length + selectedEstados.length + selectedBancos.length + (searchTerm ? 1 : 0)) > 0 && (
              <div className="flex justify-end">
                <button
                  ...
                  className="flex items-center gap-2 px-4 py-2 text-sm text-slate-600 dark:text-slate-400 hover:text-slate-900 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-lg transition-colors"
                >
            ...
            <div className="mb-3 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg p-3 flex-shrink-0">
              <div className="flex items-start gap-2">
                ...
                <div className="flex-1">
                  <h3 className="text-sm font-semibold text-red-900 dark:text-red-100">Error al cargar datos</h3>
            ...
            <div className="flex-1 overflow-auto">
              {activeTab === 'resumen' && (
                Array.isArray(data) && data.length > 0
                  ...
                  : <div className="p-8 text-center text-slate-500 dark:text-slate-400">No hay datos de resumen disponibles</div>
              )}

              {activeTab === 'historial-cambios' && (
                filteredData.length > 0
                  ? <ChangelogView changes={filteredData} />
                  : <div className="p-8 text-center text-slate-500 dark:text-slate-400">No hay cambios registrados</div>
              )}
              ...
              {activeTab === 'por-centro-gestor' && (
                filteredData.length > 0
                  ...
                  : <div className="p-8 text-center text-slate-500 dark:text-slate-400">No hay datos por centro gestor</div>
              )}

              {activeTab === 'estadisticas' && (
                data && typeof data === 'object'
                  ? <StatsView data={data} />
                  : <div className="p-8 text-center text-slate-500 dark:text-slate-400">No hay estadísticas disponibles</div>
              )}
              ...
              {canViewSolicitudesTabs && activeTab === 'historial-solicitudes' && <HistorialSolicitudesEmprestitoTab />}
            </div>
          )}
        </motion.div>
      </div>
