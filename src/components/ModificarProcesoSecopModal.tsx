...
const ModificarProcesoSecopModal: React.FC<ModificarProcesoSecopModalProps> = ({
  ...
  const validateForm = () => {
    ...
    if (!valor_publicacion || parseFloat(valor_publicacion) <= 0) {
      newErrors.valor_publicacion =
        "El valor de publicación debe ser mayor a 0";
    }
    ...
    try {
      ...
      if (!response.ok) {
        if (response.status === 404) {
          alert(
            `No se encontró el proceso con referencia: ${procesoData.referencia_proceso}`,
          );
        } else {
          alert(
            result.error || result.detail || "Error al modificar el proceso",
          );
        }
        return;
      }
    ...
    <AnimatePresence>
      ...
      >
        <motion.div
          ...
          role="dialog"
          aria-modal="true"
          aria-labelledby="modificar-proceso-secop-title"
        ...
        >
          ...
          <div className="bg-gradient-to-r from-orange-600 to-red-600 p-6 text-white">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2
                    id="modificar-proceso-secop-title"
                    className="text-2xl font-bold"
                  >
                    Modificar Valor - SECOP
                  </h2>
          ...
          <form
            ...
            className="p-4 overflow-y-auto max-h-[calc(90vh-250px)]"
          >
              ...
              <div className="grid grid-cols-2 gap-3 p-3 bg-gray-50 dark:bg-gray-800/50 rounded-lg text-sm">
                <div>
                  ...
                  <p className="font-medium text-gray-900 dark:text-white">
                    {procesoData.referencia_proceso || "N/A"}
                  </p>
                </div>
                <div>
                  <span className="text-gray-500 dark:text-gray-400">
                  ...
                  <p className="font-medium text-gray-900 dark:text-white">
                    {procesoData.valor_publicacion
                      ? new Intl.NumberFormat("es-CO", {
                          style: "currency",
                          currency: "COP",
                          minimumFractionDigits: 0,
                        }).format(procesoData.valor_publicacion)
                      : "N/A"}
                  </p>
                </div>
              </div>
