  ...
  const porPresupuesto = useMemo(() => {
    ...
    actividades.forEach((actividad) => {
      ...
      const rango = rangos.find(
        (r) => presupuesto >= r.min && presupuesto < r.max,
      );
    ...
    return (
      <div className="flex items-center justify-center p-8">
        <p className="text-gray-600 dark:text-gray-400">
          Cargando gráficos de actividades...
        </p>
      </div>
    );
  }
              ...
              <div className="relative">
                ...
                <div className="flex mt-1 space-x-1">
                  <div
                    ...
                    style={{
                      width: `${periodo.total > 0 ? (periodo.completadas / periodo.total) * 100 : 0}%`,
                    }}
                  ...
                  <div
                    ...
                    style={{
                      width: `${periodo.total > 0 ? (periodo.enProgreso / periodo.total) * 100 : 0}%`,
                    }}
                  ...
                  <div
                    ...
                    style={{
                      width: `${periodo.total > 0 ? (periodo.noIniciadas / periodo.total) * 100 : 0}%`,
                    }}
        ...
        <div className="space-y-4">
          {porPresupuesto.map((rango, index) => (
            <div key={rango.label} className="space-y-2">
              ...
              <div className="text-xs text-gray-500 dark:text-gray-400">
                {((rango.count / actividades.length) * 100).toFixed(1)}% del
                total
              </div>
            </div>
          ))}
            ...
            <div key={estado.label} className="space-y-2">
              ...
              <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-3">
                <motion.div
                  initial={{ width: 0 }}
                  animate={{
                    width: `${actividades.length > 0 ? (estado.count / actividades.length) * 100 : 0}%`,
                  }}
                  transition={{ duration: 0.8, delay: index * 0.1 }}
              ...
              <div className="text-xs text-gray-500 dark:text-gray-400">
                {actividades.length > 0
                  ? ((estado.count / actividades.length) * 100).toFixed(1)
                  : 0}
                % del total
              </div>
            </div>
