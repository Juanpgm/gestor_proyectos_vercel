...
/**
 * Utilidad para generar notificaciones de forma manual
 */
export const notifyChange = {
  /**
   * Notificar un nuevo proyecto creado
   */
  newProject: (projectName: string, projectId: string) => {
    NotificationHelpers.newProject(projectName, projectId);
  },

  /**
   * Notificar una nueva unidad creada
   */
  newUnit: (unitName: string, unitId: string, projectName: string) => {
    NotificationHelpers.newUnit(unitName, unitId, projectName);
  },

  /**
   * Notificar un nuevo contrato
   */
  newContract: (contractNumber: string, contractId: string, amount: number) => {
    NotificationHelpers.newContract(contractNumber, contractId, amount);
  },

  /**
   * Notificar una nueva actividad
   */
  newActivity: (activityName: string, activityId: string) => {
    NotificationHelpers.newActivity(activityName, activityId);
  },
  ...
  updateProject: (projectName: string, projectId: string, changes: Record<string, any>) => {
    NotificationHelpers.updateProject(projectName, projectId, changes);
  },

  /**
   * Notificar cambio de presupuesto
   */
  budgetChange: (entityName: string, oldAmount: number, newAmount: number) => {
    NotificationHelpers.budgetUpdate(entityName, oldAmount, newAmount);
  },
  ...
  statusChange: (
    ...
    NotificationHelpers.statusChange(entityName, oldStatus, newStatus, category);
  },
  ...
  deadlineWarning: (entityName: string, daysLeft: number, entityType: string) => {
    NotificationHelpers.deadlineWarning(entityName, daysLeft, entityType);
  }
};
