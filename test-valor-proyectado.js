...
async function testValorProyectado() {
  ...
  try {
    ...
    const railwaySinValor = railwayData.data.filter(
      (p) => p.valor_proyectado === null || p.valor_proyectado === undefined
    );
    ...
    if (railwayConValor.length > 0) {
      ...
      railwayConValor.slice(0, 3).forEach((p) => {
        ...
        console.log(
          `   valor_proyectado: ${
            p.valor_proyectado
          } (tipo: ${typeof p.valor_proyectado})`
        );
        console.log(
          `   nombre: ${
    ...
    try {
      ...
      if (!localData.success || !localData.data) {
        ...
        const localSinValor = localData.data.filter(
          (p) => p.valor_proyectado === null || p.valor_proyectado === undefined
        );
        ...
        if (railwayConValor.length !== localConValor.length) {
          log(
            `\n⚠️ DIFERENCIA DETECTADA: ${Math.abs(
              railwayConValor.length - localConValor.length
            )} registros`,
            "yellow"
          );
          ...
          const perdidosEnLocal = railwayConValor.filter(
            (p) => !itemsLocal.has(p.item)
          );

          if (perdidosEnLocal.length > 0) {
            log(
              `\n❌ Registros con valor_proyectado en Railway pero SIN VALOR en Local:`,
              "red"
            );
            perdidosEnLocal.slice(0, 5).forEach((p) => {
              ...
              console.log(
                `   Local   valor_proyectado: ${localRecord?.valor_proyectado}`
              );
            });
          }
        } else {
          log(
            "\n✅ Misma cantidad de registros con valor en ambos endpoints",
            "green"
          );
        }
        ...
        if (localConValor.length > 0) {
          ...
          console.log(
            `   valor_proyectado tipo: ${typeof sample.valor_proyectado}`
          );
          console.log(`   valor_proyectado valor: ${sample.valor_proyectado}`);
          console.log(
            `   ¿Es número?: ${typeof sample.valor_proyectado === "number"}`
          );
          console.log(
            `   ¿Es string?: ${typeof sample.valor_proyectado === "string"}`
          );
        }
      }
    } catch (error) {
    ...
    try {
      ...
      const sinProcesoResponse = await fetch(
        `${sinProcesoUrl}?_nocache=${Date.now()}`
      );
      const sinProcesoData = await sinProcesoResponse.json();

      if (sinProcesoData.success && sinProcesoData.data) {
        log(
          `✅ Sin Proceso Railway: ${sinProcesoData.data.length} registros`,
          "green"
        );

        const sinProcesoConValor = sinProcesoData.data.filter(
          (p) => p.valor_proyectado !== null && p.valor_proyectado !== undefined
        );
        ...
        if (sinProcesoConValor.length > 0) {
          ...
          console.log(
            `   valor_proyectado: ${
              sample.valor_proyectado
            } (${typeof sample.valor_proyectado})`
          );
        }
      }
    } catch (error) {
    ...
    log("\n🔍 Verificar:", "cyan");
    log(
      "   1. ¿El backend Railway devuelve valor_proyectado correctamente?",
      "cyan"
    );
    ...
    log(
      "   - Revisar src/app/api/emprestito/leer-tabla-proyecciones/route.ts",
      "magenta"
    );
    log(
      "   - Verificar que no haya mapeo o filtrado que elimine valor_proyectado",
      "magenta"
    );
    log(
      "   - Asegurar que NextResponse.json preserve todos los campos",
      "magenta"
    );
    ...
    log(
      "   - Verificar que Number(valor_proyectado) funcione correctamente",
      "magenta"
    );
