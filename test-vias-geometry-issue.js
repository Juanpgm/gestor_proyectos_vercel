...
async function checkViasGeometryIssue() {
  ...
  try {
    ...
    const attrResponse = await fetch('https://gestorproyectoapi-production.up.railway.app/unidades-proyecto/attributes');
    const attrData = await attrResponse.json();
    const attributes = Array.isArray(attrData) ? attrData : (attrData.attributes || attrData.data || []);
    
    // 2. Obtener geometries
    console.log('📊 2. Obteniendo GEOMETRIES...');
    const geomResponse = await fetch('https://gestorproyectoapi-production.up.railway.app/unidades-proyecto/geometry');
    ...
    const viasGeoms = geometries.filter(g => g.properties?.tipo_equipamiento === 'Vias');
    console.log(`✅ Geometries con tipo_equipamiento = "Vias": ${viasGeoms.length}`);
    
    console.log('\n' + '='.repeat(80));
    console.log('🔴 PROBLEMA DETECTADO:');
    console.log('='.repeat(80));
    console.log(`❌ ${viasAttrs.length} registros en ATTRIBUTES pero solo ${viasGeoms.length} en GEOMETRIES`);
    ...
    const geomUPIDs = new Set(viasGeoms.map(g => g.properties?.upid).filter(Boolean));
    ...
    console.log(`   • Con Estado: ${withEstado} | Sin Estado: ${withoutEstado}`);
    ...
    console.log('❌ Los registros con tipo_equipamiento = "Vias" no tienen geometrías asociadas');
    console.log('❌ Muchos registros no tienen UPID ni estado definido');
    console.log('✅ Solución: Agregar geometrías en la base de datos para estos registros');
    console.log('='.repeat(80) + '\n');
