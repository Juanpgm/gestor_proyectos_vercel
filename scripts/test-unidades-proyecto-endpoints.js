      ...
      res.on('end', () => {
        try {
          ...
          const hasData = Array.isArray(parsed) ? parsed.length > 0 : 
                         ...
                         (parsed.estados || parsed.tipos_intervencion);
          
          console.log(`✅ ${endpoint}: ${res.statusCode} - Datos: ${hasData ? 'Sí' : 'No'}`);
          console.log(`   Headers Cache: ${res.headers['cache-control'] || 'N/A'}`);
...
async function testAllEndpoints() {
  ...
  if (allOk) {
    ...
    console.log('⚠️ Algunos endpoints tienen problemas. Verifica la configuración.');
  }
