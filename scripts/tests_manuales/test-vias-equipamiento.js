...
async function testViasInFilters() {
  ...
  try {
    ...
    const hasVias = tiposEquipamiento.some((t) =>
      t.toLowerCase().includes("via")
    );
    if (hasVias) {
...
async function testViasInAttributes() {
  ...
  try {
    ...
    const viasRecords = attributes.filter(
      (attr) =>
        attr.tipo_equipamiento &&
        attr.tipo_equipamiento.toLowerCase().includes("via")
    );

    log(
      `\n🎯 Registros con "Vias": ${viasRecords.length}`,
      viasRecords.length > 0 ? "green" : "red"
    );
    ...
    if (viasRecords.length > 0) {
      ...
      const uniqueViasTypes = [
        ...new Set(viasRecords.map((r) => r.tipo_equipamiento)),
      ];
      log(`\n📌 VALORES ÚNICOS DE TIPO_EQUIPAMIENTO CON "VIA":`, "magenta");
      uniqueViasTypes.forEach((tipo) => {
        const count = viasRecords.filter(
          (r) => r.tipo_equipamiento === tipo
        ).length;
      ...
      const allTypes = [
        ...new Set(attributes.map((a) => a.tipo_equipamiento).filter(Boolean)),
      ];
      log(
        `\n📋 TODOS LOS TIPOS DE EQUIPAMIENTO (${allTypes.length}):`,
        "yellow"
      );
      allTypes.sort().forEach((tipo, index) => {
        const count = attributes.filter(
          (a) => a.tipo_equipamiento === tipo
        ).length;
...
async function testViasGeometry() {
  ...
  try {
    ...
    const viasFeatures = features.filter(
      (f) =>
        f.properties &&
        f.properties.tipo_equipamiento &&
        f.properties.tipo_equipamiento.toLowerCase().includes("via")
    );

    log(
      `\n🎯 Features con "Vias": ${viasFeatures.length}`,
      viasFeatures.length > 0 ? "green" : "red"
    );
