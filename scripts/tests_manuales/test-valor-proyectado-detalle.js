...
async function testValoresNoCero() {
  ...
  try {
    ...
    // Analizar valores
    const conValorCero = railwayProyecciones.filter(
      (p) => p.valor_proyectado === 0
    );
    const conValorPositivo = railwayProyecciones.filter(
      (p) => p.valor_proyectado > 0
    );
    const conValorNegativo = railwayProyecciones.filter(
      (p) => p.valor_proyectado < 0
    );
    const sinValor = railwayProyecciones.filter(
      (p) => p.valor_proyectado === null || p.valor_proyectado === undefined
    );
    ...
    if (conValorPositivo.length > 0) {
      ...
      topValores.forEach((p, idx) => {
        console.log(`\n   ${idx + 1}. Item: ${p.item}`);
        console.log(
          `      valor_proyectado: ${p.valor_proyectado.toLocaleString(
            "es-CO"
          )}`
        );
        console.log(
          `      Nombre: ${
            p.nombre_resumido_proceso || p.nombre_generico_proyecto || "N/A"
          }`
        );
      ...
      // Calcular total
      const totalProyectado = conValorPositivo.reduce(
        (sum, p) => sum + p.valor_proyectado,
        0
      );
      log(
        `\n   💰 Total proyectado (valores > 0): $${totalProyectado.toLocaleString(
          "es-CO"
        )}`,
        "green"
      );
      ...
      log(
        `\n⚠️ NO HAY registros con valor_proyectado > 0 en Railway`,
        "yellow"
      );
    }
    ...
    try {
      ...
      const localConValorCero = localProyecciones.filter(
        (p) => p.valor_proyectado === 0
      );
      const localConValorPositivo = localProyecciones.filter(
        (p) => p.valor_proyectado > 0
      );
      const localSinValor = localProyecciones.filter(
        (p) => p.valor_proyectado === null || p.valor_proyectado === undefined
      );
      ...
      if (localConValorPositivo.length > 0) {
        ...
        log(
          `\n⚠️ NO HAY registros con valor_proyectado > 0 en Proxy Local`,
          "yellow"
        );
      }
      ...
      if (conValorPositivo.length !== localConValorPositivo.length) {
        log(
          `\n❌ DIFERENCIA CRÍTICA: Faltan ${
            conValorPositivo.length - localConValorPositivo.length
          } registros con valores en Local`,
          "red"
        );

        // Encontrar cuáles se perdieron
        const itemsConValorRailway = new Set(
          conValorPositivo.map((p) => p.item)
        );
        const itemsConValorLocal = new Set(
          localConValorPositivo.map((p) => p.item)
        );

        const perdidos = conValorPositivo.filter(
          (p) => !itemsConValorLocal.has(p.item)
        );

        if (perdidos.length > 0) {
          log(
            `\n🔍 Items con valor en Railway pero sin valor en Local:`,
            "red"
          );
          perdidos.slice(0, 5).forEach((p) => {
            const localRecord = localProyecciones.find(
              (l) => l.item === p.item
            );
            console.log(`\n   Item: ${p.item}`);
            console.log(
              `   Railway: $${p.valor_proyectado.toLocaleString("es-CO")}`
            );
            console.log(
              `   Local:   ${localRecord?.valor_proyectado ?? "undefined"}`
            );
            console.log(`   Nombre: ${p.nombre_resumido_proceso || "N/A"}`);
          });
        }
      } else if (
        conValorPositivo.length === 0 &&
        localConValorPositivo.length === 0
      ) {
        log(`\n⚠️ AMBOS ENDPOINTS TIENEN SOLO CEROS`, "yellow");
        log(
          `   Esto podría indicar un problema en el Google Sheet de origen`,
          "yellow"
        );
      } else {
    ...
    if (conValorPositivo.length === 0) {
      ...
      log(
        "   Todos los valores de valor_proyectado son 0 en el backend Railway",
        "yellow"
      );
      log("   Esto indica que:", "yellow");
      log("   1. El Google Sheet origen tiene valores en 0", "yellow");
      log(
        "   2. O la columna valor_proyectado no está siendo leída correctamente",
        "yellow"
      );
      log("\n📋 ACCIÓN REQUERIDA:", "magenta");
      log(
        "   Verificar el Google Sheet y la lógica de lectura en el backend",
        "magenta"
      );
    } else {
