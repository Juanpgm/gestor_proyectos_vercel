  ...
  try {
    ...
    if (camposNumericos.length > 0) {
      camposNumericos.forEach((campo) => {
        ...
        console.log(
          `   ${marker} ${campo.key}: ${campo.value.toLocaleString("es-CO")}`
        );
      });
    } else {
    ...
    if ("valor_proyectado" in sampleRecord) {
      ...
      if (valor === 0) {
        ...
        log(
          `\n✅ El valor es positivo: $${valor.toLocaleString("es-CO")}`,
          "green"
        );
      ...
      log(
        `\n📊 Registros con valor_proyectado > 0: ${conValorPositivo.length} de ${data.data.length}`,
        "cyan"
      );
      ...
      if (conValorPositivo.length > 0) {
        ...
        conValorPositivo
          ...
          .forEach((p, idx) => {
            console.log(
              `   ${idx + 1}. $${p.valor_proyectado.toLocaleString(
                "es-CO"
              )} - ${p.nombre_resumido_proceso || p.item}`
            );
          });
      }
    } else {
    ...
    if (!("valor_proyectado" in sampleRecord)) {
      log(
        "\n❌ PROBLEMA CRÍTICO: El campo valor_proyectado no existe en la respuesta",
        "red"
      );
      log("\n📋 Acción requerida:", "yellow");
      log(
        "   1. Verificar el mapeo de columnas en el backend Railway",
        "yellow"
      );
      ...
      log(
        "\n✅ El campo valor_proyectado está funcionando correctamente",
        "green"
      );
    }
