...
async function testUIFlow() {
  ...
  try {
    ...
    logResult(totalInicial > 0, `Geometría cargada: ${totalInicial} features`, {
      ...
      uniqueTipos: [
        ...new Set(
          initialData.features.map((f) => f.properties?.tipo_equipamiento)
        ),
      ].length,
    });
    ...
    logStep(
      2,
      "Generar opciones del dropdown desde attributes (usando geometry como fallback)"
    );
    ...
    logResult(
      ...
      `Tipos de equipamiento extraídos: ${tiposEquipamiento.length}`,
      {
    ...
    const filteredGeometry = await fetch(
      `${API_BASE}/geometry?${params.toString()}`
    );
    const filteredData = await filteredGeometry.json();
    const totalFiltrado = filteredData.features?.length || 0;

    const todosCorrectos = filteredData.features.every(
      (f) => f.properties?.tipo_equipamiento === selectedValue
    );

    logResult(
      totalFiltrado > 0 && todosCorrectos,
      `Geometría filtrada: ${totalFiltrado} features (${(
        (totalFiltrado / totalInicial) *
        100
      ).toFixed(1)}% del total)`,
      {
    ...
    logStep(
      5,
      'Usuario selecciona múltiples: ["Bibliotecas", "CAD"] (filtrado local)'
    );
    ...
    logResult(
      ...
      `Multi-filtro LOCAL aplicado: ${locallyFiltered.length} features`,
      {
        ...
        distribucion: multipleValues.reduce((acc, val) => {
          acc[val] = locallyFiltered.filter(
            (f) => f.properties?.tipo_equipamiento === val
          ).length;
          return acc;
        }, {}),
      }
    ...
    const allTestsPassed =
      ...
      locallyFiltered.length < totalInicial &&
      totalLimpio === totalInicial;
