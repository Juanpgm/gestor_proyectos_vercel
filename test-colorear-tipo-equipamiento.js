...
const mapFile = path.join(__dirname, 'src', 'components', 'EnhancedUnidadesProyectoMap.tsx');
...
const checks = [
  {
    ...
    test: () => {
      const typeMatch = mapContent.match(/type ColoringType\s*=[\s\S]*?\|\s*'tipo_equipamiento'/);
  ...
  },
  {
    ...
    test: () => {
      const optionsMatch = mapContent.match(/\{\s*value:\s*'tipo_equipamiento',\s*label:\s*'Tipo de Equipamiento'\s*\}/);
  ...
  },
  {
    ...
    test: () => {
      const fieldMatch = mapContent.match(/case\s+'tipo_equipamiento':[\s\S]*?field\s*=\s*'tipo_equipamiento'/);
  ...
  },
  {
    ...
    test: () => {
      const popupMatch = mapContent.match(/attributeItem\.tipo_equipamiento.*Equipamiento/);
  ...
  },
  {
    ...
    test: () => {
      const conditionalMatch = mapContent.match(/\$\{attributeItem\.tipo_equipamiento\s*\?\s*`.*Equipamiento.*`\s*:\s*''\}/);
...
console.log('\n' + '='.repeat(80));
log(`RESUMEN: ${passed}/${checks.length} verificaciones pasaron`, passed === checks.length ? 'green' : 'yellow');
console.log('='.repeat(80));
