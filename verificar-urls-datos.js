...
async function verificarURLsYDatos() {
  ...
  try {
    ...
    const railwayConValor = railwayData.data.filter(p => p.valor_proyectado > 0).length;
    
    log(`   URL: ${railwayUrl}`, 'cyan');
    log(`   Registros: ${railwayData.data.length}`, 'yellow');
    log(`   Con valor > 0: ${railwayConValor}`, railwayConValor > 0 ? 'green' : 'red');
    log(`   Timestamp: ${railwayData.timestamp}`, 'cyan');
    
    if (railwayConValor > 0) {
      const sample = railwayData.data.find(p => p.valor_proyectado > 0);
      console.log(`   Ejemplo: Item ${sample.item} = $${sample.valor_proyectado.toLocaleString('es-CO')}`);
    }
  } catch (error) {
  ...
  try {
    ...
    const localConValor = localData.data.filter(p => p.valor_proyectado > 0).length;
    
    log(`   URL: ${localUrl}`, 'cyan');
    log(`   Registros: ${localData.data.length}`, 'yellow');
    log(`   Con valor > 0: ${localConValor}`, localConValor > 0 ? 'green' : 'red');
    ...
    if (localData.backend_url) {
      log(`\n   🔗 Backend URL usado por local: ${localData.backend_url}`, 'magenta');
    }
  ...
  try {
    ...
    const prodConValor = prodData.data.filter(p => p.valor_proyectado > 0).length;
    
    log(`   URL: ${prodUrl}`, 'cyan');
    log(`   Registros: ${prodData.data.length}`, 'yellow');
    log(`   Con valor > 0: ${prodConValor}`, prodConValor > 0 ? 'green' : 'red');
  ...
  log('\n📋 Si LOCAL muestra valores > 0 pero RAILWAY directo muestra 0:', 'yellow');
