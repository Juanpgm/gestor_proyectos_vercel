...
async function testDirectAPI() {
  ...
  try {
    ...
    if (data.filters) {
      ...
      console.log(`\n  - tipos_intervencion: ${filters.tipos_intervencion?.length || 0} valores`);
      console.log(`    Muestra: ${filters.tipos_intervencion?.slice(0, 3).join(', ')}`);
      
      console.log(`\n  - tipos_equipamiento: ${filters.tipos_equipamiento?.length || 0} valores`);
      console.log(`    Muestra: ${filters.tipos_equipamiento?.slice(0, 3).join(', ')}`);
      
      console.log(`\n  - centros_gestores: ${filters.centros_gestores?.length || 0} valores`);
      console.log(`    Muestra: ${filters.centros_gestores?.slice(0, 3).join(', ')}`);
      ...
      console.log(`    Muestra: ${filters.barrios_veredas?.slice(0, 3).join(', ')}`);
      
      console.log(`\n  - fuentes_financiacion: ${filters.fuentes_financiacion?.length || 0} valores`);
...
async function testProxyAPI() {
  ...
  try {
    ...
    console.log(`\n  - tipos_intervencion: ${data.tipos_intervencion?.length || 0} valores`);
    console.log(`    Muestra: ${data.tipos_intervencion?.slice(0, 3).join(', ')}`);
    
    console.log(`\n  - tipos_equipamiento: ${data.tipos_equipamiento?.length || 0} valores`);
    console.log(`    Muestra: ${data.tipos_equipamiento?.slice(0, 3).join(', ')}`);
    
    console.log(`\n  - centros_gestores: ${data.centros_gestores?.length || 0} valores`);
    console.log(`    Muestra: ${data.centros_gestores?.slice(0, 3).join(', ')}`);
    ...
    console.log(`\n  - fuentes_financiacion: ${data.fuentes_financiacion?.length || 0} valores`);
...
async function testAttributesAPI() {
  ...
  try {
    ...
    console.log(`    Muestra: ${generatedFilters.estados.slice(0, 3).join(', ')}`);
    
    console.log(`\n  - tipos_intervencion: ${generatedFilters.tipos_intervencion.length} valores`);
    console.log(`    Muestra: ${generatedFilters.tipos_intervencion.slice(0, 3).join(', ')}`);
    
    console.log(`\n  - tipos_equipamiento: ${generatedFilters.tipos_equipamiento.length} valores`);
    console.log(`    Muestra: ${generatedFilters.tipos_equipamiento.slice(0, 3).join(', ')}`);
    
    console.log(`\n  - centros_gestores: ${generatedFilters.centros_gestores.length} valores`);
    console.log(`    Muestra: ${generatedFilters.centros_gestores.slice(0, 3).join(', ')}`);
    
    console.log(`\n  - comunas: ${generatedFilters.comunas.length} valores`);
    console.log(`    Muestra: ${generatedFilters.comunas.slice(0, 5).join(', ')}`);
    
    console.log(`\n  - barrios_veredas: ${generatedFilters.barrios_veredas.length} valores`);
    console.log(`    Muestra: ${generatedFilters.barrios_veredas.slice(0, 3).join(', ')}`);
    
    console.log(`\n  - fuentes_financiacion: ${generatedFilters.fuentes_financiacion.length} valores`);
    console.log(`    Muestra: ${generatedFilters.fuentes_financiacion.join(', ')}`);
...
function compareFilters(apiFilters, proxyFilters, generatedFilters) {
  ...
  const fields = ['estados', 'tipos_intervencion', 'tipos_equipamiento', 'centros_gestores', 
                  'comunas', 'barrios_veredas', 'fuentes_financiacion', 'anos'];
    ...
    if (apiCount === proxyCount && proxyCount === generatedCount) {
      ...
      if (apiCount !== generatedCount) {
        ...
        if (inApiNotGen.length > 0) {
          console.log(`     En API pero no en Generado: ${inApiNotGen.slice(0, 5).join(', ')}`);
        }
        if (inGenNotApi.length > 0) {
          console.log(`     En Generado pero no en API: ${inGenNotApi.slice(0, 5).join(', ')}`);
        }
      }
      
      if (proxyCount !== generatedCount) {
        log(`     Proxy vs Generado: ${proxyCount} vs ${generatedCount}`, 'red');
      }
    }
  });
}
...
async function main() {
  ...
  log('Revisa las diferencias arriba para identificar problemas de caché o mapeo', 'yellow');
}
