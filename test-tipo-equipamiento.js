...
const tests = [
  ...
  },
  {
    ...
    async run() {
      ...
      const withTipoEquipamiento = features.filter(f => f.properties?.tipo_equipamiento);
      const uniqueTipos = [...new Set(withTipoEquipamiento.map(f => f.properties.tipo_equipamiento))];
      
      return {
        success: withTipoEquipamiento.length > 0,
        message: withTipoEquipamiento.length > 0
  ...
  },
  {
    ...
    async run() {
      const response = await fetch(`${API_BASE}/geometry?tipo_equipamiento=Bibliotecas`);
      const data = await response.json();
      
      const features = data.features || [];
      const allAreBibliotecas = features.every(f => 
        f.properties?.tipo_equipamiento === 'Bibliotecas'
      );
      
      return {
        success: features.length > 0 && allAreBibliotecas,
        message: features.length > 0 && allAreBibliotecas
          ? `✓ Filtro tipo_equipamiento=Bibliotecas funciona correctamente (${features.length} resultados)`
          : `✗ Filtro no aplicado correctamente (${features.length} resultados, ${allAreBibliotecas ? 'todos correctos' : 'valores mixtos'})`,
        data: {
          resultCount: features.length,
          allMatch: allAreBibliotecas,
          sampleTypes: features.slice(0, 3).map(f => f.properties?.tipo_equipamiento)
        }
      };
    }
  },
  {
    ...
    async run() {
      const response = await fetch(`${API_BASE}/geometry?tipo_equipamiento=${encodeURIComponent('Parques y zonas verdes')}`);
      const data = await response.json();
      
      const features = data.features || [];
      const allAreParques = features.every(f => 
        f.properties?.tipo_equipamiento === 'Parques y zonas verdes'
      );
      
      return {
        success: features.length > 0 && allAreParques,
        message: features.length > 0 && allAreParques
  ...
  },
  {
    ...
    async run() {
      const response = await fetch(`${API_BASE}/geometry?tipo_equipamiento=CAD`);
      const data = await response.json();
      
      const features = data.features || [];
      const allAreCAD = features.every(f => 
        f.properties?.tipo_equipamiento === 'CAD'
      );
      
      return {
        success: features.length > 0 && allAreCAD,
        message: features.length > 0 && allAreCAD
  ...
  },
  {
    ...
    async run() {
      ...
      const features = data.features || [];
      const tiposUnicos = [...new Set(features.map(f => f.properties?.tipo_equipamiento).filter(Boolean))];
      
      return {
        success: features.length > 100, // Esperamos muchos resultados sin filtro
        message: features.length > 100
...
async function runTests() {
  ...
  log(`Porcentaje de éxito: ${((passed/tests.length) * 100).toFixed(1)}%\n`, passed === tests.length ? 'green' : 'yellow');
